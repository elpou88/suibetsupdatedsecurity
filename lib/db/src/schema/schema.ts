...
export const chatRooms = pgTable("chat_rooms", {
  ...
  eventId: integer("event_id").references(() => events.id),
  name: text("name").notNull(),
  roomType: text("room_type").default("match"),
...
export const p2pChallenges = pgTable("p2p_challenges", {
  ...
  challengedWallet: text("challenged_wallet").notNull(),
  eventId: integer("event_id").references(() => events.id),
  eventName: text("event_name"),
  prediction: text("prediction").notNull(),
  amount: real("amount").notNull(),
  currency: text("currency").default("SBETS"),
  odds: real("odds").default(2.0),
  status: text("status").default("pending"),
...
export const settlementMessages = pgTable("settlement_messages", {
  id: serial("id").primaryKey(),
  ...
  txHash: text("tx_hash"),
  encryptedProof: text("encrypted_proof"),
...
export const insertP2pChallengeSchema = createInsertSchema(p2pChallenges).omit({ id: true, createdAt: true, acceptedAt: true, resolvedAt: true });
export const insertSettlementMessageSchema = createInsertSchema(settlementMessages).omit({ id: true, createdAt: true });
...
export type SettlementMessage = typeof settlementMessages.$inferSelect;
export type InsertSettlementMessage = z.infer<typeof insertSettlementMessageSchema>;

// ============================================
// P2P Betting System — Order Book Style
// Users bet against each other, no house edge
// Sui parallel execution: each match/leg settles independently
// ============================================

export const p2pBetOffers = pgTable("p2p_bet_offers", {
  id: serial("id").primaryKey(),
  creatorWallet: text("creator_wallet").notNull(),
  eventId: text("event_id").notNull(),          // External event ID (from sports API)
  ...
  sportName: text("sport_name"),
  matchDate: timestamp("match_date"),
  prediction: text("prediction").notNull(),      // "home" | "away" | "draw"
  marketType: text("market_type").default("match_winner"),
  odds: real("odds").notNull(),                  // Creator's desired odds (e.g. 2.0)
  creatorStake: real("creator_stake").notNull(), // What creator is wagering
  takerStake: real("taker_stake").notNull(),     // What taker must put up = creatorStake * (odds - 1)
  currency: text("currency").default("SUI"),
  filledStake: real("filled_stake").default(0),  // For partial fills
  status: text("status").default("open"),        // open | filled | cancelled | expired | settled
  creatorTxHash: text("creator_tx_hash"),        // Creator's payment verification
  expiresAt: timestamp("expires_at").notNull(),  // Typically when match starts
  createdAt: timestamp("created_at").defaultNow(),
  settledAt: timestamp("settled_at"),
  winner: text("winner"),                        // "creator" | "taker" | "push"
  platformFee: real("platform_fee"),
  settlementTxHash: text("settlement_tx_hash"),
  onchainOfferId: text("onchain_offer_id"),
  onchainConfigId: text("onchain_config_id"),
  refundTxHash: text("refund_tx_hash"),
  // DeepBook mirror order — posted automatically when offer is created
  deepbookClientOrderId: text("deepbook_client_order_id"),
  deepbookOrderId: text("deepbook_order_id"),
  deepbookOrderDigest: text("deepbook_order_digest"),
  // ── Sui-native feature extensions (2025-2026) ─────────────────────────────
  shareToken: text("share_token").unique(),
  suinsGated: boolean("suins_gated").default(false),
  liveOdds: boolean("live_odds").default(false),
  scoreSnapshot: text("score_snapshot"),
  matchMinute: integer("match_minute"),
});

// When a user accepts/fills a P2P offer (supports partial fills)
export const p2pBetMatches = pgTable("p2p_bet_matches", {
  id: serial("id").primaryKey(),
  offerId: integer("offer_id").references(() => p2pBetOffers.id).notNull(),
  takerWallet: text("taker_wallet").notNull(),
  stake: real("stake"),                          // Taker's stake amount
  potentialPayout: real("potential_payout"),     // Taker's payout if they win (gross)
  status: text("status").default("active"),      // active | won | lost | refunded
  takerTxHash: text("taker_tx_hash"),            // Taker's payment tx hash
  settlementTxHash: text("settlement_tx_hash"),
  settledAt: timestamp("settled_at"),
  createdAt: timestamp("created_at").defaultNow(),
  matchedAt: timestamp("matched_at").defaultNow(),
  takerFeeRate: real("taker_fee_rate").default(0.02),
  makerRebateRate: real("maker_rebate_rate").default(0),
  netFee: real("net_fee"),
  actualPayout: real("actual_payout"),
  winner: text("winner"),                        // "creator" | "taker"
  onchainMatchId: text("onchain_match_id"),
  creatorWallet: text("creator_wallet"),
  creatorStake: real("creator_stake"),
  takerStake: real("taker_stake"),
  platformFee: real("platform_fee"),
  payoutTxHash: text("payout_tx_hash"),
  // Walrus immutable receipt — archived at settlement, survives DB loss
  walrusBlobId: text("walrus_blob_id"),
  walrusReceiptJson: text("walrus_receipt_json"),
  // Checkpoint proof — Sui checkpoint that finalised this settlement TX
  checkpointSeq: text("checkpoint_seq"),
});

// P2P Parlay Offers — creator posts multi-leg parlay, taker takes all opposite sides
export const p2pParlayOffers = pgTable("p2p_parlay_offers", {
  id: serial("id").primaryKey(),
  creatorWallet: text("creator_wallet").notNull(),
  totalOdds: real("total_odds").notNull(),
  legCount: integer("leg_count").notNull().default(2),
  legsWon: integer("legs_won").default(0),
  legsLost: integer("legs_lost").default(0),
  creatorStake: real("creator_stake").notNull(),
  takerStake: real("taker_stake").notNull(),     // creatorStake * (totalOdds - 1)
  currency: text("currency").default("SUI"),
  status: text("status").default("open"),        // open | filled | cancelled | expired | settling | settled
  creatorTxHash: text("creator_tx_hash"),
  takerWallet: text("taker_wallet"),
  takerTxHash: text("taker_tx_hash"),
  expiresAt: timestamp("expires_at").notNull(),
  createdAt: timestamp("created_at").defaultNow(),
  settledAt: timestamp("settled_at"),
  winner: text("winner"),                        // "creator" | "taker"
  ...
  makerRebateRate: real("maker_rebate_rate").default(0),
  actualPayout: real("actual_payout"),
  onchainParlayId: text("onchain_parlay_id"),
  onchainConfigId: text("onchain_config_id"),
  refundTxHash: text("refund_tx_hash"),
});

// Individual legs of a P2P parlay offer
export const p2pParlayLegs = pgTable("p2p_parlay_legs", {
  id: serial("id").primaryKey(),
  parlayOfferId: integer("parlay_offer_id").references(() => p2pParlayOffers.id).notNull(),
...
// Event cursor persistence — survives server restarts so no events are missed
...
// HIP-4 volume tracking — maker rebates & taker fee tiers based on lifetime volume
export const p2pVolumeStats = pgTable("p2p_volume_stats", {
  id: serial("id").primaryKey(),
  walletAddress: text("wallet_address").notNull().unique(),
  totalVolumeMaker: real("total_volume_maker").default(0),
  totalVolumeTaker: real("total_volume_taker").default(0),
  totalBets: integer("total_bets").default(0),
  wonBets: integer("won_bets").default(0),
  totalNetPnl: real("total_net_pnl").default(0),
  lastUpdated: timestamp("last_updated").defaultNow(),
});

export const insertP2pBetOfferSchema = createInsertSchema(p2pBetOffers).omit({ id: true, createdAt: true, settledAt: true, winner: true, settlementTxHash: true, platformFee: true, filledStake: true });
export const insertP2pBetMatchSchema = createInsertSchema(p2pBetMatches).omit({ id: true, createdAt: true, settledAt: true, settlementTxHash: true });
export const insertP2pParlayOfferSchema = createInsertSchema(p2pParlayOffers).omit({ id: true, createdAt: true, settledAt: true, winner: true, settlementTxHash: true, platformFee: true, legsWon: true, legsLost: true });
export const insertP2pParlayLegSchema = createInsertSchema(p2pParlayLegs).omit({ id: true, settledAt: true });

export type P2pBetOffer = typeof p2pBetOffers.$inferSelect;
export type InsertP2pBetOffer = z.infer<typeof insertP2pBetOfferSchema>;
export type P2pBetMatch = typeof p2pBetMatches.$inferSelect;
export type InsertP2pBetMatch = z.infer<typeof insertP2pBetMatchSchema>;
export type P2pParlayOffer = typeof p2pParlayOffers.$inferSelect;
export type InsertP2pParlayOffer = z.infer<typeof insertP2pParlayOfferSchema>;
export type P2pParlayLeg = typeof p2pParlayLegs.$inferSelect;
export type InsertP2pParlayLeg = z.infer<typeof insertP2pParlayLegSchema>;
export type P2pVolumeStats = typeof p2pVolumeStats.$inferSelect;

// ─── Fantasy Football — WC 2026 Teams ────────────────────────────────────────
export const fantasyTeams = pgTable("fantasy_teams", {
  id: serial("id").primaryKey(),
  walletAddress: text("wallet_address").notNull().unique(),
  teamName: text("team_name").notNull().default("My World Cup XI"),
  starterIds: text("starter_ids").array().notNull().default([]),
  benchIds: text("bench_ids").array().notNull().default([]),
  captainId: text("captain_id").notNull().default(""),
  totalPoints: integer("total_points").notNull().default(0),
  locked: boolean("locked").notNull().default(false),
  feePaid: boolean("fee_paid").notNull().default(false),
  feeTxHash: text("fee_tx_hash"),
  devBypass: boolean("dev_bypass").notNull().default(false),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});

export type FantasyTeamRow = typeof fantasyTeams.$inferSelect;
