...
module suibets::betting {
    ...
    const EUsdsuiAlreadyInitialized: u64 = 27;
    const EUsdsuiNotInitialized: u64 = 28;
    ...
    const DEFAULT_MIN_BET_USDSUI: u64 = 1_000_000;
    const DEFAULT_MAX_BET_USDSUI: u64 = 10_000_000_000;
    ...
    const ABSOLUTE_MAX_BET_USDSUI: u64 = 100_000_000_000;
    ...
    const MAX_SINGLE_WITHDRAWAL_USDSUI: u64 = 500_000_000_000;
    ...
    public struct UsdsuiKey has copy, drop, store {}

    public struct UsdsuiState has store {
        treasury: Balance<USDSUI>,
        total_volume: u64,
    ...
    public struct OracleEcvrfKey has copy, drop, store {}

    public struct OracleEcvrfState has store {
    ...
    fun init(witness: BETTING, ctx: &mut TxContext) {
        ...
        // Claim Publisher from the one-time-witness so wallets can
        // render Bet objects as readable cards instead of raw IDs.
        let publisher = package::claim(witness, ctx);
        let mut bet_display = display::new<Bet>(&publisher, ctx);
        display::add(&mut bet_display, string::utf8(b"name"),        string::utf8(b"SuiBets Receipt"));
        display::add(&mut bet_display, string::utf8(b"description"), string::utf8(b"Decentralized sports bet on Sui — odds: {odds} bps"));
        display::add(&mut bet_display, string::utf8(b"image_url"),   string::utf8(b"https://suibets.replit.app/api/bet-image/{id}"));
        display::add(&mut bet_display, string::utf8(b"project_url"), string::utf8(b"https://suibets.replit.app"));
        display::add(&mut bet_display, string::utf8(b"creator"),     string::utf8(b"SuiBets"));
        ...
        transfer::public_transfer(bet_display, deployer);
    }
    ...
    // ============================================================
    // USDSUI BETTING FUNCTIONS
    // ============================================================

    public entry fun init_usdsui_state(
        _admin_cap: &AdminCap,
        platform: &mut BettingPlatform,
    ) {
        assert!(!dynamic_field::exists_<UsdsuiKey>(&platform.id, UsdsuiKey {}), EUsdsuiAlreadyInitialized);
        dynamic_field::add(&mut platform.id, UsdsuiKey {}, UsdsuiState {
            treasury: balance::zero(),
            total_volume: 0,
            total_potential_liability: 0,
            accrued_fees: 0,
            min_bet: DEFAULT_MIN_BET_USDSUI,
            max_bet: DEFAULT_MAX_BET_USDSUI,
        });
    }

    public entry fun place_bet_usdsui(
        platform: &mut BettingPlatform,
        payment: Coin<USDSUI>,
        event_id: vector<u8>,
        market_id: vector<u8>,
        ...
        assert!(!platform.paused, EPlatformPaused);
        assert!(dynamic_field::exists_<UsdsuiKey>(&platform.id, UsdsuiKey {}), EUsdsuiNotInitialized);
        ...
        assert!(odds >= 100, EInvalidOdds);
        assert!(odds <= 100000, EInvalidOdds);
        assert!(stake <= ABSOLUTE_MAX_BET_USDSUI, EExceedsHardMaxBet);

        let payout_u128 = ((stake as u128) * (odds as u128)) / 100u128;
        assert!(payout_u128 <= 18446744073709551615u128, EInvalidAmount);
        let potential_payout = (payout_u128 as u64);

        let usdsui_state = dynamic_field::borrow_mut<UsdsuiKey, UsdsuiState>(&mut platform.id, UsdsuiKey {});
        assert!(stake >= usdsui_state.min_bet, EExceedsMinBet);
        assert!(stake <= usdsui_state.max_bet, EExceedsMaxBet);

        let current_treasury = balance::value(&usdsui_state.treasury);
        assert!(
            current_treasury + stake >= usdsui_state.total_potential_liability + potential_payout,
            EInsufficientBalance
        );

        let payment_balance = coin::into_balance(payment);
        balance::join(&mut usdsui_state.treasury, payment_balance);

        usdsui_state.total_volume = usdsui_state.total_volume + stake;
        usdsui_state.total_potential_liability = usdsui_state.total_potential_liability + potential_payout;
    ...
    // ============================================================
    // USDSUI SETTLEMENT FUNCTIONS
    // ============================================================

    public entry fun settle_bet_usdsui(
        _oracle_cap: &OracleCap,
        platform: &mut BettingPlatform,
        bet: &mut Bet,
        won: bool,
        ...
        assert!(bet.status == STATUS_PENDING, EBetAlreadySettled);
        assert!(bet.coin_type == COIN_TYPE_USDSUI, EInvalidAmount);

        let timestamp = clock::timestamp_ms(clock);
        bet.settled_at = timestamp;

        let usdsui_state = dynamic_field::borrow_mut<UsdsuiKey, UsdsuiState>(&mut platform.id, UsdsuiKey {});
        usdsui_state.total_potential_liability = usdsui_state.total_potential_liability - bet.potential_payout;
        ...
        if (won) {
            ...
            usdsui_state.accrued_fees = usdsui_state.accrued_fees + win_fee;
            bet.platform_fee = win_fee;

            assert!(balance::value(&usdsui_state.treasury) >= net_payout, EInsufficientTreasury);

            let payout_balance = balance::split(&mut usdsui_state.treasury, net_payout);
            let payout_coin = coin::from_balance(payout_balance, ctx);
            transfer::public_transfer(payout_coin, bet.bettor);
            ...
            bet.status = STATUS_LOST;
            usdsui_state.accrued_fees = usdsui_state.accrued_fees + bet.stake;
    ...
    public entry fun settle_bet_usdsui_admin(
        _admin_cap: &AdminCap,
        platform: &mut BettingPlatform,
        bet: &mut Bet,
        won: bool,
    ...
    // ============================================================
    // VOID FUNCTIONS (USDSUI)
    // ============================================================

    public entry fun void_bet_usdsui(
        _oracle_cap: &OracleCap,
        platform: &mut BettingPlatform,
        bet: &mut Bet,
        clock: &Clock,
        ...
        let refund_amount = bet.stake;
        assert!(balance::value(&usdsui_state.treasury) >= refund_amount, EInsufficientBalance);
        let refund_balance = balance::split(&mut usdsui_state.treasury, refund_amount);
        let refund_coin = coin::from_balance(refund_balance, ctx);
        transfer::public_transfer(refund_coin, bet.bettor);
    ...
    public entry fun void_bet_usdsui_admin(
        _admin_cap: &AdminCap,
        platform: &mut BettingPlatform,
        bet: &mut Bet,
        clock: &Clock,
    ...
    public entry fun void_phantom_bet_usdsui(
        _admin_cap: &AdminCap,
        platform: &mut BettingPlatform,
        bet: &mut Bet,
        clock: &Clock,
    ...
    public entry fun claim_expired_refund_usdsui(
        platform: &mut BettingPlatform,
        bet: &mut Bet,
        clock: &Clock,
        ctx: &mut TxContext
    ...
    public entry fun deposit_liquidity_usdsui(
        _admin_cap: &AdminCap,
        platform: &mut BettingPlatform,
        coin: Coin<USDSUI>,
        clock: &Clock,
        ...
        let amount = coin::value(&coin);
        let usdsui_state = dynamic_field::borrow_mut<UsdsuiKey, UsdsuiState>(&mut platform.id, UsdsuiKey {});
        balance::join(&mut usdsui_state.treasury, coin::into_balance(coin));
    ...
    // ============================================================
    // WITHDRAWAL FUNCTIONS (USDSUI)
    // ============================================================

    public entry fun withdraw_fees_usdsui(
        _admin_cap: &AdminCap,
        platform: &mut BettingPlatform,
        amount: u64,
        clock: &Clock,
        ...
        assert!(!dynamic_field::exists_<WithdrawalLockKey>(&platform.id, WithdrawalLockKey {}), EWithdrawalsLocked);
        assert!(dynamic_field::exists_<UsdsuiKey>(&platform.id, UsdsuiKey {}), EUsdsuiNotInitialized);
        assert!(amount <= MAX_SINGLE_WITHDRAWAL_USDSUI, EWithdrawalTooLarge);
        ...
        let usdsui_state = dynamic_field::borrow_mut<UsdsuiKey, UsdsuiState>(&mut platform.id, UsdsuiKey {});
        assert!(amount <= usdsui_state.accrued_fees, EInsufficientBalance);
        assert!(balance::value(&usdsui_state.treasury) >= amount, EInsufficientTreasury);

        let fees_balance = balance::split(&mut usdsui_state.treasury, amount);
        let fees_coin = coin::from_balance(fees_balance, ctx);
        transfer::public_transfer(fees_coin, tx_context::sender(ctx));
        usdsui_state.accrued_fees = usdsui_state.accrued_fees - amount;
    ...
    public entry fun withdraw_treasury_usdsui(
        _admin_cap: &AdminCap,
        platform: &mut BettingPlatform,
        amount: u64,
        clock: &Clock,
        ...
        let usdsui_state = dynamic_field::borrow_mut<UsdsuiKey, UsdsuiState>(&mut platform.id, UsdsuiKey {});
        assert!(balance::value(&usdsui_state.treasury) >= amount, EInsufficientTreasury);
        assert!(balance::value(&usdsui_state.treasury) - amount >= usdsui_state.total_potential_liability, EInsufficientBalance);

        let treasury_balance = balance::split(&mut usdsui_state.treasury, amount);
        let treasury_coin = coin::from_balance(treasury_balance, ctx);
        transfer::public_transfer(treasury_coin, tx_context::sender(ctx));
    ...
    public entry fun update_limits_usdsui(
        _admin_cap: &AdminCap,
        platform: &mut BettingPlatform,
        min_bet: u64,
        max_bet: u64,
        ...
        assert!(dynamic_field::exists_<UsdsuiKey>(&platform.id, UsdsuiKey {}), EUsdsuiNotInitialized);
        assert!(max_bet <= ABSOLUTE_MAX_BET_USDSUI, EExceedsHardMaxBet);
        let usdsui_state = dynamic_field::borrow_mut<UsdsuiKey, UsdsuiState>(&mut platform.id, UsdsuiKey {});
        usdsui_state.min_bet = min_bet;
        usdsui_state.max_bet = max_bet;
    }
    ...
    public entry fun admin_reset_liability_usdsui(
        _admin_cap: &AdminCap,
        platform: &mut BettingPlatform,
        new_liability: u64,
    ) {
        assert!(dynamic_field::exists_<UsdsuiKey>(&platform.id, UsdsuiKey {}), EUsdsuiNotInitialized);
        let usdsui_state = dynamic_field::borrow_mut<UsdsuiKey, UsdsuiState>(&mut platform.id, UsdsuiKey {});
        usdsui_state.total_potential_liability = new_liability;
    }
    ...
    // ============================================================
    // ECVRF ORACLE FUNCTIONS
    // Transparent oracle randomness — anyone can verify the oracle
    // did not manipulate outcomes by checking the ECVRF proof on-chain.
    // ============================================================

    public entry fun set_oracle_ecvrf_public_key(
        _admin_cap: &AdminCap,
        platform: &mut BettingPlatform,
        ecvrf_public_key: vector<u8>,
        clock: &Clock,
    ) {
        // ECVRF public key is a 64-byte compressed EC point on P-256
        assert!(vector::length(&ecvrf_public_key) == 64, EInvalidPublicKey);

        if (dynamic_field::exists_<OracleEcvrfKey>(&platform.id, OracleEcvrfKey {})) {
            let state = dynamic_field::borrow_mut<OracleEcvrfKey, OracleEcvrfState>(
                &mut platform.id, OracleEcvrfKey {}
            );
            state.public_key = ecvrf_public_key;
        } else {
            dynamic_field::add(&mut platform.id, OracleEcvrfKey {}, OracleEcvrfState {
                public_key: ecvrf_public_key,
            });
        };
    ...
    // Place a SUI bet verified by ECVRF proof instead of ed25519 signature.
    // The oracle's ECVRF proof commits to (event_id + odds + expiry + bettor +
    // prediction) — an invalid proof aborts the transaction on-chain, making
    // oracle manipulation impossible to hide.
    public entry fun place_bet_ecvrf(
        platform: &mut BettingPlatform,
        payment: Coin<SUI>,
        event_id: vector<u8>,
        market_id: vector<u8>,
        ...
        assert!(!platform.paused, EPlatformPaused);
        assert!(
            dynamic_field::exists_<OracleEcvrfKey>(&platform.id, OracleEcvrfKey {}),
            EOracleNotSet
        );
        ...
        let ecvrf_state = dynamic_field::borrow<OracleEcvrfKey, OracleEcvrfState>(
            &platform.id, OracleEcvrfKey {}
        );
        // ecvrf_verify returns Some(output) on valid proof, None on invalid.
        // hash_function 0 = SHA2-256, matching the P-256-SHA256 suite.
        let output_opt = ecvrf::ecvrf_verify(&ecvrf_proof, &msg, &ecvrf_state.public_key, 0);
        assert!(option::is_some(&output_opt), EInvalidOracleSignature);
    ...
    public fun get_usdsui_stats(platform: &BettingPlatform): (u64, u64, u64, u64, u64, u64) {
        if (!dynamic_field::exists_<UsdsuiKey>(&platform.id, UsdsuiKey {})) {
            return (0, 0, 0, 0, 0, 0)
        };
        let s = dynamic_field::borrow<UsdsuiKey, UsdsuiState>(&platform.id, UsdsuiKey {});
        (balance::value(&s.treasury), s.total_volume, s.total_potential_liability, s.accrued_fees, s.min_bet, s.max_bet)
    }
