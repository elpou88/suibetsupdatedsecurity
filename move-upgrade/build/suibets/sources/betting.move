...
module suibets::betting {
    ...
    public struct AccruedFeesCorrected has copy, drop {
        platform_id: ID,
        old_fees_sui: u64,
        new_fees_sui: u64,
        old_fees_sbets: u64,
    ...
    public entry fun correct_accrued_fees(
        _admin_cap: &AdminCap,
        platform: &mut BettingPlatform,
        new_fees_sui: u64,
        new_fees_sbets: u64,
        ...
        let old_fees_sui = platform.accrued_fees_sui;
        let old_fees_sbets = platform.accrued_fees_sbets;

        assert!(new_fees_sui <= balance::value(&platform.treasury_sui), EInsufficientTreasury);
        assert!(new_fees_sbets <= balance::value(&platform.treasury_sbets), EInsufficientTreasury);

        platform.accrued_fees_sui = new_fees_sui;
        platform.accrued_fees_sbets = new_fees_sbets;

        event::emit(AccruedFeesCorrected {
            platform_id: object::id(platform),
            old_fees_sui,
            new_fees_sui,
            old_fees_sbets,
    ...
    public entry fun correct_accrued_fees_usdsui(
        _admin_cap: &AdminCap,
        platform: &mut BettingPlatform,
        new_fees: u64,
        clock: &Clock,
        ...
        assert!(new_fees <= balance::value(&usdsui_state.treasury), EInsufficientTreasury);

        let old_fees = usdsui_state.accrued_fees;
        usdsui_state.accrued_fees = new_fees;

        event::emit(AccruedFeesCorrected {
            platform_id: object::id(platform),
            old_fees_sui: 0,
            new_fees_sui: 0,
            old_fees_sbets: old_fees,
            new_fees_sbets: new_fees,
            timestamp: clock::timestamp_ms(clock),
        });
    }
