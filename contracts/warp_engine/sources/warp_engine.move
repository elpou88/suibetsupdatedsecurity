...
///
module warp_engine::warp_engine {
    ...
    public struct WarpEscrow has key {
        ...
        win_count: u64,    // cumulative wins (reserved for future TTO receive flow; always 0 in v1)
    }
    ...
    public entry fun warp_batch_marker(
        ...
        // SECURITY: voided must also be bounded — count + voided is computed below
        // and would overflow u64 if voided is near u64::MAX.  Cap at MAX_BATCH_SIZE.
        ...
        let batch_total = count + voided;   // safe: both <= 512 so max = 1024, well within u64
