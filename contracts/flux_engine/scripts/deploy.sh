#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# FLUX Engine Deploy Script
#
# Usage:
#   chmod +x contracts/flux_engine/scripts/deploy.sh
#   ./contracts/flux_engine/scripts/deploy.sh
#
# Requirements:
#   • /tmp/sui binary present (download with p2p_betting deploy instructions)
...
echo "📦 Building + publishing FLUX Engine from: $CONTRACT_DIR"
...
# ── Build ─────────────────────────────────────────────────────────────────────
echo ""
echo "🔨 Building FLUX Engine..."
$SUI_BIN move build --path "$CONTRACT_DIR" --allow-dirty

# ── Publish ───────────────────────────────────────────────────────────────────
echo ""
echo "🚀 Publishing FLUX Engine to Sui mainnet..."
...
console.log('FLUX_PACKAGE_ID='    + (pkg?.packageId    || ''));
console.log('FLUX_STATS_ID='      + (stats?.objectId    || ''));
console.log('FLUX_ADMIN_CAP_ID='  + (adminCap?.objectId || ''));
console.log('FLUX_UPGRADE_CAP_ID='+ (upgrade?.objectId  || ''));
console.log('FLUX_TX_DIGEST='     + digest);
" 2>&1)
...
echo ""
echo "═══════════════════════════════════════════════════════════════"
echo "  ⚡  FLUX Engine Deployed!"
echo "═══════════════════════════════════════════════════════════════"
echo ""
echo "  Add these environment secrets:"
echo "  FLUX_PACKAGE_ID    = ${FLUX_PACKAGE_ID:-<see above>}"
echo "  FLUX_STATS_ID      = ${FLUX_STATS_ID:-<see above>}"
echo "  FLUX_ADMIN_CAP_ID  = ${FLUX_ADMIN_CAP_ID:-<see above>}"
echo ""
[ -n "${FLUX_PACKAGE_ID:-}" ] && echo "  SuiScan: https://suiscan.xyz/mainnet/object/$FLUX_PACKAGE_ID"
[ -n "${FLUX_TX_DIGEST:-}"  ] && echo "  TX:      https://suiscan.xyz/mainnet/tx/$FLUX_TX_DIGEST"
echo "═══════════════════════════════════════════════════════════════"
