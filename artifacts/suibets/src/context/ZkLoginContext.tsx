...
import { getJsonRpcUrl, getGraphQLUrl, getDefaultNetwork } from '@/lib/suiRpcConfig';

let GOOGLE_CLIENT_ID = import.meta.env.VITE_GOOGLE_CLIENT_ID || '';
const SUI_NETWORK = getDefaultNetwork();
const SUI_PROVER_URLS = [
  'https://prover-dev.mystenlabs.com/v1',
  'https://prover.mystenlabs.com/v1',
];

const rpcClient = new SuiJsonRpcClient({ url: getJsonRpcUrl(SUI_NETWORK), network: SUI_NETWORK });
const graphqlUrl = getGraphQLUrl(SUI_NETWORK);
const readClient: any = graphqlUrl
  ? new SuiGraphQLClient({ url: graphqlUrl, network: SUI_NETWORK })
  : rpcClient;
...
export function ZkLoginProvider({ children }: { children: ReactNode }) {
  ...
  const startGoogleLogin = useCallback(async () => {
    if (!GOOGLE_CLIENT_ID) {
      const msg = 'Google login is not configured — set VITE_GOOGLE_CLIENT_ID to enable zkLogin';
      setError(msg);
      throw new Error(msg);
    }
    ...
    try {
      const { epoch } = await rpcClient.getLatestSuiSystemState();
