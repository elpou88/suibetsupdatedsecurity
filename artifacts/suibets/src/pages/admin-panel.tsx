...
import { Lock, RefreshCw, CheckCircle, XCircle, AlertCircle, Loader2, Shield, Wallet, Coins, TrendingUp, DollarSign, Flame } from 'lucide-react';
...
export default function AdminPanel() {
  ...
  const [loadingBuyback, setLoadingBuyback] = useState(false);
  const [seedingBuyback, setSeedingBuyback] = useState(false);
  const [triggeringBuyback, setTriggeringBuyback] = useState(false);
  ...
  const fetchBuybackStats = async () => {
    setLoadingBuyback(true);
    try {
      const token = getToken();
      const res = await fetch('/api/admin/buyback-stats', { headers: token ? { 'Authorization': `Bearer ${token}` } : {} });
      ...
      if (res.ok) setBuybackStats(await res.json());
      else toast({ title: 'Failed to load buyback stats', variant: 'destructive' });
    } catch (e: any) { toast({ title: 'Error loading buyback', description: e.message, variant: 'destructive' }); }
    finally { setLoadingBuyback(false); }
  };
  ...
  const seedBuybackFromBets = async (count: number) => {
    setSeedingBuyback(true);
    try {
      const token = getToken();
      const res = await fetch('/api/admin/buyback-seed-from-bets', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...(token ? { 'Authorization': `Bearer ${token}` } : {}) },
        body: JSON.stringify({ count }),
      });
      if (res.status === 401) { setIsAuthenticated(false); sessionStorage.removeItem('adminToken'); toast({ title: 'Session expired', variant: 'destructive' }); return; }
      if (!res.ok) { toast({ title: 'Seed Failed', description: `Server returned ${res.status}`, variant: 'destructive' }); return; }
      const data = await res.json();
      if (data.success) {
        toast({ title: 'Buyback Seeded', description: `Seeded from ${data.betsProcessed} bets: ${data.totalSeeded.sui.toFixed(4)} SUI + ${data.totalSeeded.sbets.toFixed(2)} SBETS` });
        setBuybackStats(data.pool);
      } else {
        toast({ title: 'Seed Failed', description: data.message, variant: 'destructive' });
      }
    } catch (e: any) { toast({ title: 'Error', description: e.message, variant: 'destructive' }); }
    finally { setSeedingBuyback(false); }
  };

  const triggerManualBuyback = async () => {
    setTriggeringBuyback(true);
    try {
      const token = getToken();
      const res = await fetch('/api/admin/buyback-trigger', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...(token ? { 'Authorization': `Bearer ${token}` } : {}) },
      });
      if (res.status === 401) { setIsAuthenticated(false); sessionStorage.removeItem('adminToken'); toast({ title: 'Session expired', variant: 'destructive' }); return; }
      if (!res.ok) { toast({ title: 'Buyback Failed', description: `Server returned ${res.status}`, variant: 'destructive' }); return; }
      const data = await res.json();
      toast({ title: data.success ? 'Buyback Triggered' : 'Buyback Skipped', description: data.message });
      fetchBuybackStats();
    } catch (e: any) { toast({ title: 'Error', description: e.message, variant: 'destructive' }); }
    finally { setTriggeringBuyback(false); }
  };
  ...
  return (
    <Layout>
      <div className="container mx-auto px-4 py-8 max-w-7xl">
                        ...
                        <table className="w-full text-xs">
                          ...
                          <tbody>
                            {(unsettledBets.bets || [])
                              ...
                              .map((bet: any) => (
                                <tr key={bet.id} className="border-b border-gray-800 text-gray-300 hover:bg-gray-800/50">
                                  <td className="py-2 px-2 font-mono text-xs">{bet.id}</td>
                                  <td className="py-2 px-2">
                                    <Badge variant="outline" className={
                                  ...
                                  <td className="py-2 px-2 max-w-[120px] truncate" title={bet.eventName || `${bet.homeTeam} vs ${bet.awayTeam}`}>
                                  ...
                                  <td className="py-2 px-2 text-right">{Number(bet.betAmount).toFixed(2)}</td>
        ...
        {/* Buyback & Revenue Section - Visible when authenticated */}
        {isAuthenticated && (
          <Card className="bg-gradient-to-r from-orange-900/30 to-red-900/30 border-orange-500/30 mb-8">
            <CardHeader className="flex flex-row items-center justify-between gap-4">
              <CardTitle className="text-xl text-white flex items-center gap-2">
              ...
              <div className="flex gap-2">
                <Button size="sm" variant="outline" onClick={fetchBuybackStats} disabled={loadingBuyback}
                  className="border-orange-500/50 text-orange-400 hover:bg-orange-900/30" data-testid="button-refresh-buyback">
                  <RefreshCw className={`w-4 h-4 mr-1 ${loadingBuyback ? 'animate-spin' : ''}`} /> Buyback
                </Button>
                <Button size="sm" variant="outline" onClick={fetchRevenueController} disabled={loadingRevenue}
                  className="border-green-500/50 text-green-400 hover:bg-green-900/30" data-testid="button-refresh-revenue">
                  <RefreshCw className={`w-4 h-4 mr-1 ${loadingRevenue ? 'animate-spin' : ''}`} /> Revenue
                </Button>
              </div>
            </CardHeader>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <div className="bg-black/40 rounded-lg p-3">
                      <p className="text-gray-400 text-xs">Pending SUI Pool</p>
                      <p className="text-lg font-bold text-orange-400" data-testid="text-pending-sui">{(buybackStats.pendingPoolSui || 0).toFixed(4)}</p>
                    </div>
                    <div className="bg-black/40 rounded-lg p-3">
                      <p className="text-gray-400 text-xs">Pending SBETS Pool</p>
                      <p className="text-lg font-bold text-orange-400" data-testid="text-pending-sbets">{(buybackStats.pendingPoolSbets || 0).toFixed(2)}</p>
                    </div>
                    <div className="bg-black/40 rounded-lg p-3">
                      <p className="text-gray-400 text-xs">Total SBETS Burned</p>
                      <p className="text-lg font-bold text-red-400" data-testid="text-total-burned">{(buybackStats.totalSbetsBurned || 0).toFixed(2)}</p>
                    </div>
                    <div className="bg-black/40 rounded-lg p-3">
                      <p className="text-gray-400 text-xs">Total Burns</p>
                      <p className="text-lg font-bold text-cyan-400" data-testid="text-total-burns">{buybackStats.totalBurns || 0}</p>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    <div className="bg-black/40 rounded-lg p-3">
                      <p className="text-gray-400 text-xs">Daily SUI Spent</p>
                      <p className="text-sm font-bold text-white">{(buybackStats.dailyBuybackSui || 0).toFixed(4)} / {buybackStats.config?.dailyCap || 50}</p>
                    </div>
                    <div className="bg-black/40 rounded-lg p-3">
                      <p className="text-gray-400 text-xs">Total SUI Spent</p>
                      <p className="text-sm font-bold text-white">{(buybackStats.totalBuybackSui || 0).toFixed(4)}</p>
                    </div>
                    <div className="bg-black/40 rounded-lg p-3">
                      <p className="text-gray-400 text-xs">Last Burn</p>
                      <p className="text-sm font-bold text-white">{buybackStats.lastSwapTime ? new Date(buybackStats.lastSwapTime).toLocaleString() : 'Never'}</p>
                    </div>
                  </div>
                </div>
              ) : (
                <p className="text-gray-500 text-sm">Click "Buyback" to load stats</p>
              )}

              <div className="flex flex-wrap gap-3 pt-2 border-t border-orange-500/20">
                <Button size="sm" onClick={() => seedBuybackFromBets(5)} disabled={seedingBuyback}
                  className="bg-orange-600 hover:bg-orange-700 text-white" data-testid="button-seed-5">
                  {seedingBuyback ? <Loader2 className="w-4 h-4 mr-1 animate-spin" /> : <Flame className="w-4 h-4 mr-1" />}
                  Seed from Last 5 Bets
                </Button>
                <Button size="sm" onClick={() => seedBuybackFromBets(20)} disabled={seedingBuyback}
                  className="bg-orange-700 hover:bg-orange-800 text-white" data-testid="button-seed-20">
                  {seedingBuyback ? <Loader2 className="w-4 h-4 mr-1 animate-spin" /> : <Flame className="w-4 h-4 mr-1" />}
                  Seed from Last 20 Bets
                </Button>
                <Button size="sm" onClick={triggerManualBuyback} disabled={triggeringBuyback}
                  className="bg-red-600 hover:bg-red-700 text-white" data-testid="button-trigger-buyback">
                  {triggeringBuyback ? <Loader2 className="w-4 h-4 mr-1 animate-spin" /> : <Flame className="w-4 h-4 mr-1" />}
                  Trigger Buyback Now
                </Button>
              </div>
              ...
              {revenueController && (
                <div className="space-y-3 pt-4 border-t border-green-500/20">
                  <h3 className="text-white font-semibold flex items-center gap-2">
                    <TrendingUp className="w-5 h-5 text-green-400" /> Revenue Controller
                  </h3>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <div className="bg-black/40 rounded-lg p-3">
                      <p className="text-gray-400 text-xs">All-Time Revenue</p>
                      <p className="text-sm font-bold text-green-400">{(revenueController.allTime?.grossRevenue || 0).toFixed(4)}</p>
                    </div>
                    <div className="bg-black/40 rounded-lg p-3">
                      <p className="text-gray-400 text-xs">Buyback Allocated</p>
                      <p className="text-sm font-bold text-orange-400">{(revenueController.allTime?.buybackAllocated || 0).toFixed(4)}</p>
                    </div>
                    <div className="bg-black/40 rounded-lg p-3">
                      <p className="text-gray-400 text-xs">Net Revenue</p>
                      <p className="text-sm font-bold text-white">{(revenueController.allTime?.netRevenue || 0).toFixed(4)}</p>
                    </div>
                    <div className="bg-black/40 rounded-lg p-3">
                      <p className="text-gray-400 text-xs">Today Revenue</p>
                      <p className="text-sm font-bold text-cyan-400">{(revenueController.today?.grossRevenue || 0).toFixed(4)}</p>
                    </div>
                  </div>
                  {revenueController.recentEntries?.length > 0 && (
                      ...
                      <table className="w-full text-xs">
                        <thead>
                          <tr className="text-gray-500 border-b border-gray-800">
                            ...
                            <th className="text-right p-1">Buyback</th>
                            <th className="text-right p-1">Net</th>
                            <th className="text-right p-1">Time</th>
                          </tr>
                        </thead>
                        <tbody>
                          {revenueController.recentEntries.slice(0, 20).map((entry: any, i: number) => (
                            <tr key={i} className="text-gray-300 border-b border-gray-900" data-testid={`revenue-entry-${i}`}>
                              <td className="p-1">{entry.source}</td>
                              <td className="text-right p-1">{Number(entry.grossAmount).toFixed(4)}</td>
                              <td className="text-right p-1 text-orange-400">{Number(entry.buybackAmount).toFixed(4)}</td>
                              <td className="text-right p-1">{Number(entry.netAmount).toFixed(4)}</td>
                              <td className="text-right p-1 text-gray-500">{new Date(entry.createdAt).toLocaleTimeString()}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
