...
const SPORT_KEYWORDS: Record<string, string[]> = {
  ...
  'mma-ufc':  ['mma', 'ufc', 'boxing', 'bellator'],
...
function matchesSportSlug(offer: any, slug: string): boolean {
...
export default function SportPage() {
  ...
  const noLiveSports = ['esports', 'wwe', 'wwe-entertainment', 'entertainment', 'horse-racing'];
  ...
  const sportOffers = (allOffers as any[]).filter(
    (o: any) => (o.status === 'open' || o.status === 'partial') && matchesSportSlug(o, sportSlugLower)
  );
  const openParlays = (allParlays as any[]).filter((p: any) => p.status === 'open');
  ...
  return (
    <div className={`${getSportGradient()} text-white min-h-screen`}>
      <div className="container py-6">
        ...
        {/* ── P2P Open Market ─────────────────────────────────────────── */}
        {(sportOffers.length > 0 || openParlays.length > 0) && (
          <div className="mt-2 mb-8">
            <div className="flex items-center justify-between mb-4">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-cyan-400" />
                  <span className="text-xs font-black tracking-widest uppercase text-cyan-400">P2P Open Market</span>
                </div>
                <p className="text-gray-500 text-xs">Bet against other users — no house edge</p>
              </div>
              <button
            ...
            {/* Single-match P2P Offers */}
            {sportOffers.length > 0 && (
              <div className="mb-5">
                <div className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2 ml-1">
                  ⚔️ Single Offers ({sportOffers.length})
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {sportOffers.slice(0, 6).map((offer: any) => {
                    ...
                    return (
                      <button
                        ...
                        onMouseLeave={e => (e.currentTarget.style.background = 'rgba(0,255,255,0.03)')}
                      >
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-white font-bold text-xs truncate pr-2">
                            {offer.homeTeam} vs {offer.awayTeam}
                          </span>
                          <span className="text-cyan-400 font-black text-xs whitespace-nowrap">
                            {Number(offer.odds || 0).toFixed(2)}x
                          </span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-gray-400 text-[11px]">
                            🏅 <span className="text-white font-bold">{pred}</span>
                            <span className="text-gray-600 ml-1">· {offer.leagueName || ''}</span>
                          </span>
                          <span className="text-gray-500 text-[11px]">
                            Win {takerWin.toFixed(2)} {cur}
                          </span>
                        </div>
                      </button>
                    );
                ...
                {sportOffers.length > 6 && (
                  <button onClick={() => navigate('/p2p')} className="mt-2 text-xs text-cyan-400 hover:underline ml-1">
                    +{sportOffers.length - 6} more offers →
                  </button>
                )}
              </div>
            )}
            ...
            {openParlays.length > 0 && (
              <div>
                <div className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2 ml-1">
                  🎯 Parlay Offers ({openParlays.length})
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {openParlays.slice(0, 4).map((parlay: any) => {
                    const takerWin = ((Number(parlay.creatorStake) || 0) + (Number(parlay.takerStake) || 0)) * 0.98;
                    const legs: any[] = Array.isArray(parlay.legs) ? parlay.legs : [];
                    return (
                      <button
                        key={parlay.id}
                        onClick={() => navigate('/p2p?tab=parlays')}
                      ...
                      >
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span className="text-purple-400 font-black text-xs">
                            {Number(parlay.totalOdds || 0).toFixed(2)}x
                          </span>
                        </div>
                        <div className="text-gray-400 text-[11px] truncate mb-1">
                          {legs.slice(0, 2).map((l: any) => `${l.homeTeam} vs ${l.awayTeam}`).join(' · ')}
                          {legs.length > 2 && <span className="text-gray-600"> +{legs.length - 2} more</span>}
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-gray-600 text-[11px]">{parlay.currency} · All legs win</span>
                          <span className="text-gray-500 text-[11px]">Win {takerWin.toFixed(2)} {parlay.currency}</span>
                        </div>
                      </button>
                    );
                  })}
                </div>
                {openParlays.length > 4 && (
                  <button onClick={() => navigate('/p2p?tab=parlays')} className="mt-2 text-xs text-purple-400 hover:underline ml-1">
                    +{openParlays.length - 4} more parlays →
                  </button>
                )}
              </div>
            )}
