...
import { Clock, Calendar, Filter, ChevronRight, Zap, ArrowRight } from 'lucide-react';
...
export default function UpcomingEventsPage() {
  ...
  // Fetch all open P2P offers (to count per event)
  const { data: allP2POffers = [] } = useQuery<any[]>({
    queryKey: ['/api/p2p/offers', 'upcoming-count'],
    queryFn: () =>
  ...
  // Build a map: eventId → offer count
  const p2pCountByEvent = allP2POffers.reduce<Record<string, number>>((acc, o) => {
    if (o.eventId) acc[o.eventId] = (acc[o.eventId] || 0) + 1;
    return acc;
  }, {});
    ...
    <Layout title="Upcoming Events">
      <div className="min-h-screen bg-[#0b1618] text-white py-6">
        <div className="container mx-auto px-4">
          ...
          {/* P2P Hub Banner */}
          {totalP2POffers > 0 && (
            <div className="mb-6 rounded-xl border border-yellow-500/30 bg-yellow-500/5 p-4 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold text-yellow-300 text-sm">
                    {totalP2POffers} P2P offer{totalP2POffers !== 1 ? 's' : ''} open right now
                  </p>
                  <p className="text-xs text-gray-400">Bet peer-to-peer at user-set odds — no house edge</p>
                </div>
              </div>
              <Button
                size="sm"
                className="bg-yellow-500 hover:bg-yellow-400 text-black font-semibold shrink-0 gap-1"
                onClick={() => setLocation('/p2p')}
              >
                Open P2P Hub <ArrowRight className="h-3.5 w-3.5" />
              </Button>
            </div>
          )}
                  ...
                  <CardHeader className="border-b border-[#1e3a3f]">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        ...
                        {p2pCountByEvent[event.id] > 0 && (
                          <button
                            onClick={() => setLocation('/p2p')}
                            className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-yellow-500/10 border border-yellow-500/30 text-yellow-400 text-xs font-semibold hover:bg-yellow-500/20 transition-colors"
                          >
                            <Zap className="h-2.5 w-2.5" />
                            {p2pCountByEvent[event.id]} P2P
                          </button>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        {p2pCountByEvent[event.id] > 0 ? (
                          <span className="text-xs text-yellow-300 font-medium">
                            {p2pCountByEvent[event.id]} P2P offer{p2pCountByEvent[event.id] !== 1 ? 's' : ''} open
                          </span>
                        ) : (
                          <span className="text-xs text-gray-500">Be first to post a P2P offer</span>
                        )}
                      </div>
                      <div className="flex gap-1.5 shrink-0">
                        {p2pCountByEvent[event.id] > 0 && (
                          <button
                            ...
                            className="text-xs px-2.5 py-1 rounded-md bg-yellow-500/10 border border-yellow-500/25 text-yellow-400 hover:bg-yellow-500/20 transition-colors"
                          >
                            View
                          </button>
                        )}
                        <button
                          ...
                          className="text-xs px-2.5 py-1 rounded-md bg-yellow-500 hover:bg-yellow-400 text-black font-semibold transition-colors flex items-center gap-1"
                        >
