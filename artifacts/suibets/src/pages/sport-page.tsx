...
const SportEventCard = ({ event, sportId }: { event: Event, sportId: number }) => {
  ...
  if (!markets || markets.length === 0) {
    ...
    markets = sportMarketsAdapter.enhanceMarketsForSport(markets as any, sportId, event.homeTeam, event.awayTeam, { home: event.homeOdds, draw: event.drawOdds, away: event.awayOdds });
  }
              ...
              {primaryMarket.outcomes.slice(0, 2).map((outcome, i) => (
                ...
                >
                  <div className="flex flex-col">
                    ...
                    <span className="font-bold">{Math.round(100 / outcome.odds)}%</span>
                  </div>
                </Button>
              ))}
            </div>
              ...
              <div className="mt-1">
                ...
                >
                  <div className="flex flex-col">
                    ...
                    <span className="font-bold">{Math.round(100 / primaryMarket.outcomes[2].odds)}%</span>
                  </div>
                </Button>
              </div>
            )}
...
const getSportIdBySlug = (slug: string): number => {
  const sportMap: Record<string, number> = {
    ...
    'ice-hockey': SportIds.HOCKEY,
    'handball': SportIds.HANDBALL,
    ...
    'mma-ufc': SportIds.MMA_UFC,
    'formula-1': SportIds.FORMULA_1,
    'formula1': SportIds.FORMULA_1,
    'f1': SportIds.FORMULA_1,
    'cycling': SportIds.CYCLING,
    'american-football': SportIds.AMERICAN_FOOTBALL,
    'nfl': SportIds.AMERICAN_FOOTBALL,
    'afl': SportIds.AFL,
    'horse-racing': SportIds.HORSE_RACING,
    'motogp': SportIds.MOTOGP,
    'moto-gp': SportIds.MOTOGP,
...
const getSportNameById = (id: number): string => {
  const sportNames: Record<number, string> = {
    ...
    [SportIds.HOCKEY]: 'Ice Hockey',
    [SportIds.HANDBALL]: 'Handball',
    ...
    [SportIds.AFL]: 'AFL',
    [SportIds.HORSE_RACING]: 'Horse Racing',
    [SportIds.MOTOGP]: 'MotoGP',
