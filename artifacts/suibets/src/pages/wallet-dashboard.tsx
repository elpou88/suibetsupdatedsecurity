...
export default function WalletDashboardPage() {
  ...
  const wonBets = userBets.filter((b: any) => b.status === 'won' || b.status === 'paid_out').length;
  const lostBets = userBets.filter((b: any) => b.status === 'lost').length;
  const cashedOut = userBets.filter((b: any) => b.status === 'cashed_out').length;
  const totalStaked = userBets.reduce((acc: number, b: any) => acc + (b.stake || 0), 0);
  const totalWon = userBets.filter((b: any) => b.status === 'won' || b.status === 'paid_out').reduce((acc: number, b: any) => acc + (b.potentialWin || 0), 0);
