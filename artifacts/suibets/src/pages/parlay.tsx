...
import { useCurrentAccount, useSuiClientQuery } from '@/lib/dapp-kit-compat';
...
export default function ParlayPage() {
  ...
  const handlePlaceParlay = async () => {
    ...
    try {
      ...
      let betObjectIds: string[] = [];

      // Single bets use the real eventId directly.
      // Multi-leg parlays use a composite parlay_ event ID so the oracle
      // can sign the combined odds as a single atomic unit.
      const isSingle = parlayLegs.length === 1;
      const parlayEventId = isSingle
        ? String(parlayLegs[0].eventId)
        : `parlay_${Date.now()}_${parlayLegs.map(l => l.eventId).join('~')}`;
      const parlayMarketId = isSingle ? (parlayLegs[0].marketId || 'match_winner') : 'parlay_combined';
      const parlayPrediction = isSingle
        ? parlayLegs[0].selection
        : parlayLegs.map(l => `${l.eventName}: ${l.selection}`).join(' | ');

      const onChainResult = await placeBetOnChain({
        eventId: parlayEventId,
        marketId: parlayMarketId,
        prediction: parlayPrediction.slice(0, 500),
        betAmount: stakeAmount,
        odds: totalOdds,
        walrusBlobId: '',
        coinType: betCurrency as 'SUI' | 'SBETS',
      ...
      betObjectIds = betObjectId ? [betObjectId] : [];
      ...
      if (response?.ok) {
        const betType = parlayLegs.length === 1 ? 'Bet' : `${parlayLegs.length}-Leg Parlay`;
        const txLink = `https://suiscan.xyz/mainnet/tx/${txDigest}`;
        toast({ 
          title: `${betType} Placed On-Chain!`, 
          description: (
            <div className="flex flex-col gap-1">
              <span>
                {parlayLegs.length === 1 ? parlayLegs[0].selection : `${parlayLegs.length} legs — 1 atomic TX`} @ {totalOdds.toFixed(2)}x
              </span>
      ...
      <nav className="bg-black/40 backdrop-blur-md border-b border-cyan-900/30 px-4 py-3">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            <Link href="/wallet-dashboard" className="text-gray-400 hover:text-cyan-400 text-sm font-medium" data-testid="nav-dashboard">Dashboard</Link>
