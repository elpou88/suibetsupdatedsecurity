...
import WorldCupFantasy from "@/components/fantasy/WorldCupFantasy";
import { Link } from "wouter";
import { useQuery, useQueryClient } from "@tanstack/react-query";
import { ChevronDown, ChevronUp, ArrowLeft, Search, Clock, Globe, Star, Target, Users, Shield, Zap, TrendingUp, Award, BarChart3, XCircle, Play, Radio, Newspaper, Table2, ExternalLink, RefreshCw, Loader2, X } from "lucide-react";
...
export default function WorldCupPage() {
  ...
  const [pageTab, setPageTab] = useState<'futures' | 'matches' | 'groups' | 'news' | 'fantasy'>('futures');
  ...
  const handleSelectTeam = (selection: FuturesSelection) => {
    ...
    const settlementDate = futuresData?.settlementDate || '2026-07-19T23:59:59Z';
    addBet({
      id: selection.id,
  ...
  return (
    <div className="min-h-screen bg-[#060a10] text-white pb-20 md:pb-0">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="mb-6 flex gap-2 overflow-x-auto pb-2 scrollbar-hide -mx-1 px-1">
          <button
          ...
          >
            <Radio size={16} />
            <span className="whitespace-nowrap">Live & Upcoming</span>
            {wcMatches.length > 0 && (
              <span className="bg-red-500 text-white text-[10px] font-black px-1.5 py-0.5 rounded-full">{wcMatches.length}</span>
            )}
          </button>
          <button
            onClick={() => setPageTab('fantasy')}
            className={`flex-shrink-0 flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold transition-all ${
              pageTab === 'fantasy'
                ? 'bg-gradient-to-r from-green-500 to-emerald-500 text-white shadow-lg shadow-green-500/20'
                : 'bg-white/[0.03] text-gray-500 border border-white/5 hover:text-gray-300'
            }`}
            data-testid="page-tab-fantasy"
          >
            <Users size={16} />
            <span className="whitespace-nowrap">Fantasy</span>
            <span className="bg-green-500 text-white text-[9px] font-black px-1.5 py-0.5 rounded-full">NEW</span>
          </button>
        </div>
        ...
        {pageTab === 'fantasy' && (
          <WorldCupFantasy walletAddress={currentAccount?.address || zkLoginAddress || undefined} />
        )}
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-[#060a10]/97 backdrop-blur-xl border-t border-cyan-500/15">
        <div className="flex items-stretch" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
          {([
            { id: 'futures' as const, Icon: TrendingUp,  label: 'Futures',  color: 'cyan'  },
            { id: 'groups'  as const, Icon: Table2,      label: 'Groups',   color: 'cyan'  },
            { id: 'news'    as const, Icon: Newspaper,   label: 'News',     color: 'cyan'  },
            { id: 'matches' as const, Icon: Radio,       label: 'Live',     color: 'red'   },
            { id: 'fantasy' as const, Icon: Users,       label: 'Fantasy',  color: 'green' },
          ]).map(({ id, Icon, label, color }) => {
            const isActive = pageTab === id;
            const activeText  = color === 'green' ? 'text-green-400' : color === 'red' ? 'text-red-400' : 'text-cyan-400';
            const activeBg    = color === 'green' ? 'bg-green-500/10' : color === 'red' ? 'bg-red-500/10' : 'bg-cyan-500/10';
            const accentColor = color === 'green' ? 'bg-green-400' : color === 'red' ? 'bg-red-400' : 'bg-cyan-400';
            return (
              <button
                key={id}
                onClick={() => { setPageTab(id); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
                className={`flex-1 flex flex-col items-center justify-center gap-1 py-2.5 transition-all relative ${
                  isActive ? `${activeText} ${activeBg}` : 'text-gray-500 active:text-gray-300'
                }`}
              >
                {isActive && (
                  <span className={`absolute top-0 left-1/4 right-1/4 h-0.5 rounded-b-full ${accentColor}`} />
                )}
                <div className="relative">
                  <Icon size={18} />
                  {id === 'matches' && wcMatches.length > 0 && (
                    <span className="absolute -top-1.5 -right-2.5 bg-red-500 text-white text-[8px] font-black w-3.5 h-3.5 rounded-full flex items-center justify-center leading-none">
                      {wcMatches.length > 9 ? '9+' : wcMatches.length}
                    </span>
                  )}
                  {id === 'fantasy' && !isActive && (
                    <span className="absolute -top-1.5 -right-3.5 bg-green-500 text-white text-[7px] font-black px-1 py-px rounded-full leading-none">NEW</span>
                  )}
                </div>
...
type MarketSelection = { prediction: string; label: string; odds: number };
type MarketDef = { id: string; label: string; icon: string; selections: (home: string, away: string) => MarketSelection[] };

const WC_MARKETS: MarketDef[] = [
  {
    id: '1x2', label: 'Match Result', icon: '⚽',
    selections: (home, away) => [
      { prediction: 'home',  label: home,  odds: 2.10 },
      { prediction: 'draw',  label: 'Draw', odds: 3.30 },
      { prediction: 'away',  label: away,  odds: 3.60 },
    ],
  },
  {
    id: 'double_chance', label: 'Double Chance', icon: '🎯',
    selections: (home, away) => [
      { prediction: 'home_or_draw', label: `${home} or Draw`,       odds: 1.35 },
      { prediction: 'home_or_away', label: 'Either Team Wins',      odds: 1.55 },
      { prediction: 'away_or_draw', label: `${away} or Draw`,       odds: 2.20 },
    ],
  },
  {
    id: 'goals', label: 'Goals Over/Under', icon: '📊',
    selections: () => [
      { prediction: 'over_0.5',  label: 'Over 0.5',  odds: 1.09 },
      { prediction: 'under_0.5', label: 'Under 0.5', odds: 8.00 },
      { prediction: 'over_1.5',  label: 'Over 1.5',  odds: 1.42 },
      { prediction: 'under_1.5', label: 'Under 1.5', odds: 2.90 },
      { prediction: 'over_2.5',  label: 'Over 2.5',  odds: 2.05 },
      { prediction: 'under_2.5', label: 'Under 2.5', odds: 1.80 },
      { prediction: 'over_3.5',  label: 'Over 3.5',  odds: 3.40 },
      { prediction: 'under_3.5', label: 'Under 3.5', odds: 1.32 },
      { prediction: 'over_4.5',  label: 'Over 4.5',  odds: 5.50 },
      { prediction: 'under_4.5', label: 'Under 4.5', odds: 1.12 },
    ],
  },
  {
    id: 'btts', label: 'Both Teams Score', icon: '🔥',
    selections: () => [
      { prediction: 'btts_yes', label: 'Yes', odds: 1.85 },
      { prediction: 'btts_no',  label: 'No',  odds: 1.95 },
    ],
  },
  {
    id: 'asian_handicap', label: 'Asian Handicap', icon: '⚖️',
    selections: (home, away) => [
      { prediction: 'home_-0.5', label: `${home} -0.5`, odds: 2.00 },
      { prediction: 'away_+0.5', label: `${away} +0.5`, odds: 1.88 },
      { prediction: 'home_-1.5', label: `${home} -1.5`, odds: 3.50 },
      { prediction: 'away_+1.5', label: `${away} +1.5`, odds: 1.33 },
      { prediction: 'away_-0.5', label: `${away} -0.5`, odds: 2.20 },
      { prediction: 'home_+0.5', label: `${home} +0.5`, odds: 1.75 },
    ],
  },
  {
    id: 'halftime', label: 'Half-Time Result', icon: '⏱️',
    selections: (home, away) => [
      { prediction: 'ht_home', label: `${home} HT`, odds: 2.50 },
      { prediction: 'ht_draw', label: 'Draw HT',    odds: 2.70 },
      { prediction: 'ht_away', label: `${away} HT`, odds: 4.50 },
    ],
  },
  {
    id: 'correct_score', label: 'Correct Score', icon: '🏆',
    selections: () => [
      { prediction: 'score_1-0', label: '1 - 0', odds: 6.50  },
      { prediction: 'score_0-0', label: '0 - 0', odds: 9.00  },
      { prediction: 'score_1-1', label: '1 - 1', odds: 6.00  },
      { prediction: 'score_2-0', label: '2 - 0', odds: 9.00  },
      { prediction: 'score_0-1', label: '0 - 1', odds: 8.00  },
      { prediction: 'score_2-1', label: '2 - 1', odds: 9.00  },
      { prediction: 'score_1-2', label: '1 - 2', odds: 11.00 },
      { prediction: 'score_0-2', label: '0 - 2', odds: 11.00 },
      { prediction: 'score_2-2', label: '2 - 2', odds: 14.00 },
      { prediction: 'score_3-0', label: '3 - 0', odds: 18.00 },
      { prediction: 'score_0-3', label: '0 - 3', odds: 22.00 },
      { prediction: 'score_3-1', label: '3 - 1', odds: 19.00 },
      { prediction: 'score_1-3', label: '1 - 3', odds: 23.00 },
      { prediction: 'score_3-2', label: '3 - 2', odds: 30.00 },
    ],
  },
  {
    id: 'clean_sheet', label: 'Clean Sheet', icon: '🛡️',
    selections: (home, away) => [
      { prediction: 'cs_home', label: `${home} CS`, odds: 2.30 },
      { prediction: 'cs_away', label: `${away} CS`, odds: 2.80 },
    ],
  },
];

// ─── Per-match offer creation panel ──────────────────────────────────────────
...
function MatchOfferPanel({
  ...
  const packageId  = contractData?.packageId  ?? P2P_FALLBACK_PACKAGE_ID;
  const configId   = contractData?.configId   ?? P2P_FALLBACK_CONFIG_ID;
  const registryId = contractData?.registryId ?? P2P_FALLBACK_REGISTRY_ID;

  const oddsNum  = parseFloat(odds)  || 0;
  const stakeNum = parseFloat(stake) || 0;
  const takerStake = stakeNum > 0 && oddsNum > 1 ? +(stakeNum * (oddsNum - 1)).toFixed(6) : 0;
  const potentialPayout = stakeNum + takerStake > 0 ? +((stakeNum + takerStake) * 0.98).toFixed(6) : 0;
  ...
  const handlePost = async () => {
    ...
    if (stakeNum <= 0) {
      toast({ title: 'Enter stake', description: 'Stake must be greater than 0.', variant: 'destructive' });
      return;
    }
    if (oddsNum < 1.01) {
    ...
    try {
      ...
      const oddsBps = BigInt(Math.round(oddsNum * 10_000));
      const matchDate = match.startTime || match.matchDate || match.date || match.commence_time;
      const matchMs   = matchDate ? new Date(matchDate).getTime() : 0;
      const expiresAtMs = BigInt(matchMs > Date.now() ? matchMs : Date.now() + 72 * 3600 * 1000);

      const eventId   = String(match.id || match.eventId || '');
      const eventName = match.title || match.name || `${homeTeam} vs ${awayTeam}`;
      ...
      tx.moveCall({
        ...
        arguments: [
          ...
          tx.pure(bcs.vector(bcs.u8()).serialize(toBytes(selection.prediction))),
          tx.pure(bcs.vector(bcs.u8()).serialize(toBytes('match_market'))),
          tx.pure.u64(oddsBps),
      ...
      toast({ title: 'Approve in wallet', description: `Locking ${stakeNum} SUI in P2P contract escrow…` });
      ...
      if (!digest) throw new Error('No transaction digest returned. Please try again.');
      ...
      if (!res.ok) {
        ...
        throw new Error(err.message || 'Failed to register offer');
      }

      toast({ title: 'Offer posted!', description: `${selection.label} @ ${oddsNum.toFixed(2)}x — live on the P2P board.` });
      qc.invalidateQueries({ queryKey: ['/api/p2p/offers'] });
      onClose();
    } catch (err: any) {
      toast({ title: 'Failed to post offer', description: err.message, variant: 'destructive' });
    } finally {
      setPosting(false);
    }
  };
  ...
  return (
    <div className="border-t border-cyan-500/20 bg-[#070c16]/95 p-4" onClick={e => e.stopPropagation()}>
      <div className="flex items-center justify-between mb-3">
        <div>
          <div className="text-white text-sm font-bold">Create P2P Offer</div>
          <div className="text-gray-400 text-xs mt-0.5">
            Your pick: <span className="text-cyan-300 font-semibold">{selection.label}</span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        <div>
          <label className="text-gray-500 text-xs mb-1 block">Your Odds</label>
          <input
            type="number"
            min="1.01"
            step="0.05"
            ...
            className="w-full bg-white/[0.05] border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/20"
          />
        </div>
        <div>
          <label className="text-gray-500 text-xs mb-1 block">Your Stake (SUI)</label>
          <input
            type="number"
            min="0.001"
            step="0.5"
      ...
      <div className="flex items-center justify-between text-xs mb-3 bg-white/[0.03] rounded-lg px-3 py-2 border border-white/5">
        <div>
          <span className="text-gray-500">Taker needs: </span>
          <span className="text-yellow-400 font-bold">{takerStake.toFixed(4)} SUI</span>
        </div>
        <div>
          <span className="text-gray-500">If you win: </span>
          <span className="text-green-400 font-bold">+{potentialPayout.toFixed(4)} SUI</span>
        </div>
      </div>

      <button
        onClick={handlePost}
        disabled={posting || stakeNum <= 0 || oddsNum < 1.01}
        className="w-full py-2.5 rounded-lg text-sm font-bold transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
        style={{ background: 'linear-gradient(135deg, #00ffff22, #00bcd422)', border: '1px solid rgba(0,255,255,0.35)', color: '#00ffff' }}
      >
        {posting ? (
          <><Loader2 size={14} className="animate-spin" /> Posting…</>
        ) : (
          <>⚡ Post P2P Offer — {stakeNum > 0 ? `${stakeNum} SUI` : 'Enter stake'}</>
        )}
      </button>

      {!currentAccount?.address && (
        <p className="text-center text-gray-600 text-[11px] mt-2">Connect wallet to post offers</p>
      )}
    </div>
  );
}

// ─── Single match market card ─────────────────────────────────────────────────

function WCMatchCard({ match, index }: { match: any; index: number }) {
  const [expanded, setExpanded]         = useState(false);
  const [activeMarket, setActiveMarket] = useState('1x2');
  const [selectedSel, setSelectedSel]   = useState<MarketSelection | null>(null);
  const [showStream, setShowStream]     = useState(false);

  const isLive    = match.status === 'live' || match.isLive;
  const streamUrl = match.streamUrl || match.embed;

  const title = match.title || match.name || 'World Cup Match';
  let homeTeam = match.homeTeam || match.team1 || match.teams?.home?.name || '';
  let awayTeam = match.awayTeam || match.team2 || match.teams?.away?.name || '';
  if (!homeTeam || !awayTeam) {
    const parts = title.split(/\s+vs\.?\s+/i);
    homeTeam = homeTeam || parts[0]?.trim() || 'Home';
    awayTeam = awayTeam || parts[1]?.trim() || '';
  }

  const market = WC_MARKETS.find(m => m.id === activeMarket) ?? WC_MARKETS[0];
  const selections = market.selections(homeTeam, awayTeam);

  const handleSelectSel = (sel: MarketSelection) => {
    setSelectedSel(prev => (prev?.prediction === sel.prediction ? null : sel));
  };
  ...
  return (
    <div
      ...
      style={{
        ...
        borderColor: expanded ? 'rgba(0,255,255,0.25)' : 'rgba(255,255,255,0.07)',
        boxShadow: expanded ? '0 0 20px rgba(0,255,255,0.06)' : 'none',
      }}
    ...
    >
      ...
      <div className="p-4">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className="text-gray-600 text-xs">{match.category || match.league || 'FIFA World Cup 2026'}</span>
          </div>
        ...
        <div className="flex items-center justify-between gap-3 mb-3">
          <div className="flex items-center gap-3 flex-1 min-w-0">
            ...
            {awayTeam && <span className="text-gray-600 text-xs flex-shrink-0">vs</span>}
            {awayTeam && <span className="text-white font-bold text-sm truncate">{awayTeam}</span>}
          </div>
          <div className="flex items-center gap-2 flex-shrink-0">
            {streamUrl && (
              <button
                ...
                style={{ color: '#f87171', borderColor: 'rgba(248,113,113,0.3)', background: 'rgba(248,113,113,0.07)' }}
                data-testid={`btn-watch-stream-${index}`}
              >
                <Play size={10} />
                {showStream ? 'Hide' : 'Stream'}
              </button>
            )}
            <button
              onClick={() => { setExpanded(!expanded); setSelectedSel(null); }}
              className="flex items-center gap-1 text-[11px] font-bold px-3 py-1 rounded-lg border transition-all"
              style={{
                color: expanded ? '#00ffff' : '#9ca3af',
                borderColor: expanded ? 'rgba(0,255,255,0.35)' : 'rgba(255,255,255,0.1)',
                background: expanded ? 'rgba(0,255,255,0.08)' : 'rgba(255,255,255,0.03)',
              }}
              data-testid={`btn-bet-markets-${index}`}
            >
              {expanded ? <><ChevronUp size={11} /> Hide</> : <><Target size={11} /> Bet</>}
            </button>
          </div>
        </div>
      </div>
      ...
      {expanded && (
        <div className="border-t border-white/5">
          {/* Market tabs */}
          <div className="flex gap-1 p-3 pb-0 overflow-x-auto scrollbar-hide">
            {WC_MARKETS.map(m => (
              <button
                key={m.id}
                onClick={() => { setActiveMarket(m.id); setSelectedSel(null); }}
                className="flex items-center gap-1 text-[11px] font-bold px-2.5 py-1.5 rounded-lg flex-shrink-0 transition-all whitespace-nowrap"
                style={{
                  background: activeMarket === m.id ? 'rgba(0,255,255,0.12)' : 'rgba(255,255,255,0.03)',
                  border: `1px solid ${activeMarket === m.id ? 'rgba(0,255,255,0.35)' : 'rgba(255,255,255,0.07)'}`,
                  color: activeMarket === m.id ? '#00ffff' : '#6b7280',
                }}
              >
                <span>{m.icon}</span>
                <span>{m.label}</span>
          ...
          {/* Selections grid */}
          <div className="p-3">
            <div className={`grid gap-2 ${activeMarket === 'goals' || activeMarket === 'correct_score' ? 'grid-cols-2 sm:grid-cols-3' : activeMarket === 'asian_handicap' ? 'grid-cols-2' : 'grid-cols-3'}`}>
              {selections.map(sel => {
                const active = selectedSel?.prediction === sel.prediction;
                return (
                  <button
                    key={sel.prediction}
                    onClick={() => handleSelectSel(sel)}
                    className="flex flex-col items-center justify-center py-3 px-2 rounded-xl border transition-all hover:scale-[1.02] active:scale-[0.98]"
                    style={{
                      background: active
                        ? 'linear-gradient(135deg, rgba(0,255,255,0.15), rgba(0,188,212,0.1))'
                        : 'rgba(255,255,255,0.03)',
                      borderColor: active ? 'rgba(0,255,255,0.5)' : 'rgba(255,255,255,0.07)',
                      boxShadow: active ? '0 0 12px rgba(0,255,255,0.15)' : 'none',
                    }}
                  >
                    <span className="text-gray-400 text-[11px] leading-tight text-center mb-1.5 line-clamp-2">{sel.label}</span>
                    <span
                      className="text-base font-black leading-none"
                      style={{ color: active ? '#00ffff' : '#e5e7eb' }}
                    >
            ...
            {!selectedSel && (
              <p className="text-center text-gray-700 text-[11px] mt-3">
                Select a market above to create a P2P offer
              </p>
            )}
          </div>
          ...
          {selectedSel && (
            <MatchOfferPanel
              ...
              homeTeam={homeTeam}
              awayTeam={awayTeam}
              selection={selectedSel}
              onClose={() => setSelectedSel(null)}
            />
          )}
        </div>
      )}
    ...
    <div className="mb-8 space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-1">
        {matches.map((match: any, index: number) => (
          <WCMatchCard key={match.id || index} match={match} index={index} />
        ))}
      </div>
    </div>
  );
