...
export default function MobileSidebar() {
  ...
  const { data: rawSports } = useQuery<Sport[]>({
    queryKey: ['/api/sports'],
    queryFn: () => fetch('/api/sports').then(r => r.json()).then(d => Array.isArray(d) ? d : []),
  });
  const sports: Sport[] = Array.isArray(rawSports) ? rawSports : [];
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex-grow overflow-y-auto scrollbar-hidden space-y-1">
            ...
            <Link href="/tokenomics">
              <div 
                className={`flex items-center px-4 py-3 mx-2 rounded ${
                  location === '/tokenomics' || location === '/revenue'
                    ? 'bg-yellow-500 text-black' 
                    : 'text-yellow-400 hover:bg-[#042A30]'
                }`}
                onClick={() => handleNavigation('/tokenomics')}
                data-testid="link-mobile-tokenomics"
              >
                <div className="w-8 h-8 mr-3 flex items-center justify-center">
                  <span className="text-lg">◈</span>
                </div>
                <span className={location === '/tokenomics' || location === '/revenue' ? 'font-semibold' : ''}>Revenue</span>
              </div>
            </Link>
