...
const MAIN_CATEGORY = {
  ...
  sports: [
    ...
    { id: 0, name: '⚔️ P2P Bets', slug: 'p2p', icon: 'chart', highlight: false }
  ]
};
...
export default function Sidebar() {
  ...
  const { data: rawApiSports } = useQuery<Sport[]>({
    queryKey: ['/api/sports'],
    queryFn: () => fetch('/api/sports').then(r => r.json()).then(d => Array.isArray(d) ? d : []),
  });
  const apiSports: Sport[] = Array.isArray(rawApiSports) ? rawApiSports : [];
