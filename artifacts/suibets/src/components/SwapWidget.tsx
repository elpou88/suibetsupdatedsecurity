...
function createAggregator() {
  try {
    const suiMainnetClient = new SuiJsonRpcClient({ url: getJsonRpcUrl("mainnet"), network: "mainnet" });
    if (_bluefinSdk?.BluefinConfig?.setSuiClient) {
      _bluefinSdk.BluefinConfig.setSuiClient(suiMainnetClient);
    }
    return new MetaAg({
      slippageBps: 100,
      providers: {
