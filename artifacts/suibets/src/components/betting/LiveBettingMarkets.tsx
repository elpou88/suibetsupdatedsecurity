...
/**
 * Per-event sub-component so the useLiveClock hook can run inside the map.
 * Renders the live clock badge + score that ticks every second in real time.
 */
function LiveMatchClock({ event, sportId }: { event: Event; sportId: number }) {
  const clock = useLiveClock(event, sportId);
  return (
    <div className="text-xs bg-[#1e3a3f] text-cyan-300 px-2 py-1 rounded flex items-center gap-1">
      <span className="w-1.5 h-1.5 bg-red-500 rounded-full animate-pulse shrink-0" />
      <span className="font-mono font-bold">{clock}</span>
      {event.score && event.score !== '0-0' && (
        <span className="border-l border-cyan-700 pl-1 ml-0.5">{event.score}</span>
      )}
    </div>
  );
}
...
function LiveBettingMarkets() {
  ...
  const { 
    ...
    refetchInterval: 10000, // Poll every 10 s — matches the server WS broadcast cadence
    retry: 2, 
    retryDelay: 1000,
    staleTime: 8000,
    refetchOnWindowFocus: true, // Immediately re-check when user returns to tab
    placeholderData: (previousData) => previousData ?? [], // Keep previous data during in-flight refetch
  });
  
  // Process and normalize events.
  // No stale-cache fallback here — if the server returned [] it means no live
  // events exist and we should show the empty state, not ghost-events.
  ...
  // Loading state — only show spinner on initial load, not background refetches
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <LiveMatchClock event={event} sportId={sportIdNum} />
                      </div>
                    </div>
                  </CardHeader>
