 ...
 */
function isMatchLikelyFinished(bet: any): boolean {
  if (bet.status === 'won' || bet.status === 'lost' || bet.status === 'paid_out' || bet.status === 'cashed_out') return true;
  if (bet.parlayLegLost) return true;
  const isParlay = bet.betType === 'parlay' || (bet.prediction && bet.prediction.startsWith('['));
  if (isParlay && bet.result) {
    try {
      const legResults = JSON.parse(bet.result);
      if (Array.isArray(legResults) && legResults.some((l: any) => l.won === false)) return true;
    } catch {}
  }
  if (isParlay) return false;
  const startTime = bet.eventStartTime || bet.startTime;
  if (startTime) {
    const matchStart = new Date(startTime).getTime();
    const now = Date.now();
    const threeHoursMs = 3 * 60 * 60 * 1000;
    if (now > matchStart + threeHoursMs) return true;
  }
  return false;
}
...
export function BetHistory() {
  ...
  const getParlaySelections = (bet: any): { eventName: string; selection: string; odds: number; eventId?: string; marketId?: string; homeTeam?: string; awayTeam?: string; leagueName?: string; matchDate?: string }[] => {
    try {
          ...
          {isLoading ? (
            ...
            <div className="space-y-4 mt-4">
              {filteredBets.map((bet: any) => (
                <Card key={bet.id} className="premium-row-hover bg-[#0b1618] border-[#1e3a3f] overflow-hidden">
                          ...
                          <>
                            ...
                            <div className="mt-2 space-y-1">
                              {getParlaySelections(bet).map((leg, idx) => {
                                  ...
                                  if (bet.status === 'won' || bet.status === 'paid_out') {
                                    ...
                                    try {
                                      ...
                                      if (Array.isArray(parsed)) {
                                        const legEventId = leg.eventId || leg.externalEventId || '';
                                        const matchedResult = parsed.find((r: any) => String(r.eventId).trim() === String(legEventId).trim()) || parsed[idx];
                                        if (matchedResult) {
                                          legWonResult = matchedResult.won;
                                          if (legWonResult === null || legWonResult === undefined) {
                                            legColor = 'text-gray-400';
                                ...
                                return (
                                <div key={idx} className={`text-xs ${legColor} flex items-start gap-2 min-w-0`}>
                                  <span className={`w-4 h-4 ${bgColor} rounded-full flex items-center justify-center ${numColor} flex-shrink-0 mt-0.5`}>
                                    {idx + 1}
                                  </span>
                                  <span className="truncate min-w-0 flex-1">{leg.eventName || 'Match'}: {leg.selection} @ {leg.odds?.toFixed(2)}</span>
                                </div>
                                );
                              })}
                            </div>
                          ...
                          <div className="text-right">
                            {bet.walrusBlobId.startsWith('local_') ? (
                              <a
                                ...
                                title="Receipt stored locally — uploading to Walrus in the background"
                              >
                              ...
                              <div className="flex items-center gap-2 justify-end flex-wrap">
                                ...
                                <span className="text-gray-600">·</span>
                                <a
                                  href={`https://walruscan.com/mainnet/blob/${bet.walrusBlobId}`}
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  className="inline-flex items-center gap-1 text-xs bg-gradient-to-r from-purple-500/20 to-blue-500/20 border border-purple-500/30 text-purple-300 hover:text-purple-200 px-2 py-0.5 rounded-full transition-colors"
                                  data-testid={`btn-verify-walrus-${bet.id}`}
                                >
