...
export function PromotionBanner() {
  const { data: rawPromotions } = useQuery<Promotion[]>({
    queryKey: ['/api/promotions'],
    queryFn: () => fetch('/api/promotions').then(r => r.json()).then(d => Array.isArray(d) ? d : (Array.isArray(d?.promotions) ? d.promotions : [])),
  });
  const promotions: Promotion[] = Array.isArray(rawPromotions) ? rawPromotions : [];
