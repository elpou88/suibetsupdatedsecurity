...
export function useSuiNSName(address: string | undefined | null): string | null {
...
export function useSuiNSNameWithStatus(address: string | undefined | null): {
  name: string | null;
  isLoading: boolean;
} {
  const { data: name, isLoading } = useQuery<string | null>({
    queryKey: ['/api/suins/name', address],
    queryFn: async () => {
      if (!address || !address.startsWith('0x')) return null;
      const res = await fetch(`/api/suins/resolve?address=${encodeURIComponent(address)}`);
