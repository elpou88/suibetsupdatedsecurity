  ...
  private transformBaseballApiGames(games: any[], isLive: boolean): SportEvent[] {
    return games.map((game: any, index: number) => {
      try {
        ...
        // Skip games where teams haven't been determined yet (playoff bracket placeholders)
        const isTbd = (name: string) => !name || name.trim().toUpperCase() === 'TBD' || name.trim() === '' || name === 'Home Team' || name === 'Away Team';
        if (isTbd(homeTeam) || isTbd(awayTeam)) {
          console.log(`[BaseballService] Skipping TBD game: ${homeTeam} vs ${awayTeam}`);
          return null;
        }
    ...
    }).filter((g): g is SportEvent => g !== null);
  }
}
