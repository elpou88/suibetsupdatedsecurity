...
class SettlementWorkerService {
    ...
    try {
      ...
      for (const match of finishedMatches) {
        ...
        const betsForMatch = pendingBets.filter(bet => {
          ...
          // GUARD: Skip name matching only when BOTH IDs share the same sport-specific
          // prefix (e.g. both ice-hockey_api_*) — provably different games from same source.
          // Pure numeric IDs from different providers (API-Sports vs ESPN) MUST fall through
          // to team-name matching — the IDs differ for the same real-world match.
          const sameIdFormat = (() => {
          ...
          if (!sameIdFormat && bet.homeTeam && bet.awayTeam) {
            const normAcc = (s: string) => s.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().trim();
            const betHome = normAcc(bet.homeTeam);
            const betAway = normAcc(bet.awayTeam);
            const matchHome = normAcc(match.homeTeam);
            const matchAway = normAcc(match.awayTeam);
  ...
  /**
   * Force-settle a specific parlay bet by injecting explicit per-leg match results.
   * Used by the admin endpoint when API_SPORTS_KEY is unavailable or match lookup fails.
   * betId: the parlay's database row id (number) or wurlus bet id (string like "parlay-...")
   * legResults: array of { eventId, homeTeam, awayTeam, homeScore, awayScore } one per leg
   */
  public async forceSettleParlayBet(
    betId: number | string,
    legResults: Array<{ eventId: string; homeTeam: string; awayTeam: string; homeScore: number; awayScore: number }>
  ): Promise<{ success: boolean; message: string; outcome?: string }> {
    try {
      const allPending = await storage.getAllBets('pending');
      const allConfirmed = await storage.getAllBets('confirmed');
      const allBets = [...allPending, ...allConfirmed];

      const bet = allBets.find(b =>
        String(b.id) === String(betId) ||
        (b as any).wurlusBetId === String(betId) ||
        (b as any).walrusBetId === String(betId)
      );

      if (!bet) {
        return { success: false, message: `Bet ${betId} not found or not in pending/confirmed state` };
      }

      const mapped: UnsettledBet = {
        id: bet.id,
        eventId: bet.eventId || '',
        externalEventId: (bet as any).externalEventId || (bet as any).wurlusBetId || String(bet.id),
        homeTeam: bet.homeTeam || '',
        awayTeam: bet.awayTeam || '',
        prediction: (bet as any).selection || bet.prediction || '',
        odds: bet.odds,
        stake: (bet as any).stake || (bet as any).betAmount,
        potentialWin: (bet as any).potentialWin || (bet as any).potentialPayout,
        userId: (bet as any).walletAddress || String((bet as any).userId || ''),
        currency: bet.currency || 'SUI',
        betObjectId: (bet as any).betObjectId || undefined,
        status: bet.status,
        giftedTo: (bet as any).giftedTo || undefined,
        placedAt: (typeof (bet as any).createdAt === 'string' ? (bet as any).createdAt : (bet as any).createdAt?.toISOString?.()) || undefined,
        matchDate: undefined,
      };

      if (!this.isParlayBet(mapped)) {
        return { success: false, message: `Bet ${betId} is not recognised as a parlay — cannot force-settle with this method` };
      }

      // Build injected match map from provided legResults
      const injectedMap = new Map<string, FinishedMatch>();
      for (const lr of legResults) {
        const winner: 'home' | 'away' | 'draw' =
          lr.homeScore > lr.awayScore ? 'home' :
          lr.awayScore > lr.homeScore ? 'away' : 'draw';
        injectedMap.set(String(lr.eventId).trim(), {
          eventId: lr.eventId,
          homeTeam: lr.homeTeam,
          awayTeam: lr.awayTeam,
          homeScore: lr.homeScore,
          awayScore: lr.awayScore,
          winner,
          status: 'finished'
        });
      }
      ...
      // Parse legs
      let legs: Array<{ eventId: string; prediction: string; marketId?: string; outcomeId?: string }>;
      const pred = mapped.prediction || '';
      ...
      if (pred.startsWith('[') && pred.includes('"eventId"')) {
        ...
        legs = this.parsePipeSeparatedParlay(mapped);
      } else {
        return { success: false, message: `Cannot parse parlay legs from bet ${betId} — unknown format` };
      }

      let allLegsFinished = true;
      let anyLegLost = false;
      const resolvedLegs: { eventId: string; prediction: string; won: boolean; match: FinishedMatch }[] = [];

      for (const leg of legs) {
        const eventId = String(leg.eventId).trim();
        const match = injectedMap.get(eventId);
        if (!match) {
          console.warn(`⚠️ Force-settle parlay: no injected result for leg ${eventId}`);
          allLegsFinished = false;
          continue;
        }
        const legResult = this.evaluateLegPrediction(leg.prediction || (leg as any).selection || '', match, leg.marketId, leg.outcomeId);
        if (legResult === null) {
          return { success: false, message: `Cannot evaluate prediction "${leg.prediction}" for leg ${eventId}` };
        }
        if (!legResult) anyLegLost = true;
        resolvedLegs.push({ eventId, prediction: leg.prediction, won: legResult, match });
      }

      if (!allLegsFinished && !anyLegLost) {
        return { success: false, message: `Not all legs had injected results — provide a result for every leg` };
      }

      const parlayWon = !anyLegLost;
      const firstMatch = resolvedLegs[0]?.match;
      if (!firstMatch) {
        return { success: false, message: 'No leg results resolved — check injected data' };
      }

      const legResultsJson = JSON.stringify(resolvedLegs.map(lr => ({
        eventId: lr.eventId,
        prediction: lr.prediction,
        won: lr.won,
        score: `${lr.match.homeScore}-${lr.match.awayScore}`,
        teams: `${lr.match.homeTeam} vs ${lr.match.awayTeam}`
      })));
      try { await storage.updateBetResult(bet.id, legResultsJson); } catch {}

      console.log(`🎰 FORCE PARLAY SETTLE: bet ${betId} → ${parlayWon ? 'WON' : 'LOST'} (${resolvedLegs.filter(l => l.won).length}/${legs.length} legs won)`);

      this.settledBetIds.delete(bet.id); // ensure we can re-settle

      const modifiedBet: UnsettledBet = { ...mapped, status: parlayWon ? 'won' : 'pending' };
      if (parlayWon) {
        await this.settleBetsForMatch(firstMatch, [modifiedBet]);
      } else {
        await this.settleParlaySingleBet(mapped, firstMatch, false);
      }

      return { success: true, message: `Parlay ${betId} force-settled as ${parlayWon ? 'WON' : 'LOST'}`, outcome: parlayWon ? 'won' : 'lost' };
    } catch (error: any) {
      console.error(`❌ forceSettleParlayBet error:`, error);
      return { success: false, message: error.message };
    }
  }
  
  private async _doCheckAndSettleBets(forceRefresh = false) {
    const apiKey = ''; // API_SPORTS_KEY disabled — football uses ESPN (free, no key needed)
    console.log(`🔍 SettlementWorker: Checking for finished matches... [Football→ESPN ✅ free | Paid API key: disabled]`);
    ...
    try {
      ...
      for (const match of finishedMatches) {
        ...
        const betsForMatch = singleBets.filter(bet => {
          ...
          // GUARD: Skip name matching only when BOTH IDs share the same sport-specific
          // prefix (e.g. both ice-hockey_api_*) — they are provably different games of
          // the same teams from the same API source.
          // NOTE: Pure numeric IDs (football/soccer from API-Sports vs ESPN) are from
          // DIFFERENT sources and MUST fall through to team-name matching — the numeric
          // IDs are different for the same match depending on the data provider.
          const sameIdFormat2 = (() => {
            if (!betExtId || !matchId) return false;
            const p1 = betExtId.match(/^([a-z-]+_api_)/)?.[1];
            const p2 = matchId.match(/^([a-z-]+_api_)/)?.[1];
      ...
      if (unmatchedSingles.length > 0) {
        ...
        for (const bet of unmatchedSingles) {
          ...
          if (isHorseRacing) {
            ...
            console.log(`⚽ FOOTBALL BET: ${bet.id} (${extId}) — direct fixture lookup unavailable (paid API removed). Will settle via finished-matches list.`);
          }
  ...
  private parsePipeSeparatedParlay(bet: UnsettledBet): Array<{ eventId: string; prediction: string; marketId?: string; outcomeId?: string; homeTeam?: string; awayTeam?: string }> {
    const legs: Array<{ eventId: string; prediction: string; marketId?: string; outcomeId?: string; homeTeam?: string; awayTeam?: string }> = [];
    ...
    try {
      ...
      for (let i = 0; i < Math.min(eventIds.length, predParts.length); i++) {
        ...
        const rawPrediction = colonIdx !== -1 ? fullPred.slice(colonIdx + 1).trim() : fullPred;
        // Strip trailing "@ X.XX" odds suffix that may have been included in the stored prediction string
        const prediction = rawPrediction.replace(/\s*@\s*[\d.]+\s*$/, '').trim();
        ...
        // Extract homeTeam / awayTeam from "TeamA vs TeamB: prediction" format
        let homeTeam: string | undefined;
        let awayTeam: string | undefined;
        const eventNamePart = colonIdx !== -1 ? fullPred.slice(0, colonIdx).trim() : '';
        if (eventNamePart) {
          const vsMatch = eventNamePart.match(/^(.+?)\s+vs\.?\s+(.+)$/i);
          if (vsMatch) {
        ...
        legs.push({ eventId, prediction, marketId, outcomeId, homeTeam, awayTeam });
      }
    } catch (error) {
  ...
  private async settleParlayBets(parlayBets: UnsettledBet[], finishedMatches: FinishedMatch[]) {
    ...
    for (const bet of parlayBets) {
      try {
        ...
        // Fast-path: if the result field already has a stored lost leg, settle immediately
        if ((bet as any).result) {
          try {
            const storedResult = JSON.parse((bet as any).result);
            if (Array.isArray(storedResult) && storedResult.some((l: any) => l.won === false)) {
              console.log(`⚡ FAST-SETTLE: Parlay ${bet.id.slice(0, 10)}... stored result has lost leg — settling as LOST immediately`);
              const firstLeg = storedResult[0];
              const dummyMatch: FinishedMatch = {
                eventId: firstLeg?.eventId || bet.externalEventId || String(bet.id),
                homeTeam: firstLeg?.teams?.split(' vs ')?.[0] || bet.homeTeam || '',
                awayTeam: firstLeg?.teams?.split(' vs ')?.[1] || bet.awayTeam || '',
                homeScore: 0, awayScore: 0, winner: 'draw' as const, status: 'finished'
              };
              this.settledBetIds.delete(bet.id);
              await this.settleParlaySingleBet(bet, dummyMatch, false);
              continue;
            }
          } catch {}
        }
        ...
        let unsettleableLegCount = 0; // legs whose market can't be evaluated (void just that leg)
        ...
        for (const leg of legs) {
          ...
          // Team-name fallback: search finishedMatchMap by home/away team names.
          // This catches ESPN-sourced legs whose numeric IDs differ from the paid-API IDs
          // stored in finishedMatchMap, but whose team names still match.
          if (!match) {
            const legHome = ((leg.homeTeam || '') as string).trim();
            const legAway = ((leg.awayTeam || '') as string).trim();
            if (legHome && legAway) {
              const norm = (s: string) =>
                s.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/\s+/g, ' ').trim();
              const nHome = norm(legHome);
              const nAway = norm(legAway);
              for (const m of finishedMatchMap.values()) {
                const mHome = norm(m.homeTeam);
                const mAway = norm(m.awayTeam);
                if ((mHome === nHome && mAway === nAway) || (mHome === nAway && mAway === nHome)) {
                  match = m;
                  finishedMatchMap.set(eventId, match);
                  console.log(`🔍 Team-name match for parlay leg ${eventId}: ${match.homeTeam} vs ${match.awayTeam} (${match.homeScore}-${match.awayScore})`);
                  break;
                }
              }
            }
          ...
          const legResult = this.evaluateLegPrediction(prediction, match, leg.marketId, leg.outcomeId);
          
          if (legResult === null) {
            // Market is unsettleable (period-specific, handicap, spread, etc.).
            // Industry standard: void just this leg and continue — do NOT hold the
            // whole parlay pending until it gets voided after 48h.
            console.log(`⏭️ Parlay leg VOIDED (unsettleable market): prediction="${prediction}" for ${match.homeTeam} vs ${match.awayTeam} — voiding this leg only, parlay continues`);
            unsettleableLegCount++;
            // Don't set allLegsFinished = false — this leg is "resolved" (as void)
            continue;
          }
          
          legResults.push({ eventId, prediction, won: legResult, match });
          ...
          if (bet.stake > 0) {
            ...
            try {
              if (bet.currency === 'USDSUI') {
                ...
                console.log(`⚠️ PARLAY VOID: bet ${bet.id} voided — no off-chain refund, on-chain void only`);
              }
            } catch (refundErr: any) {
        ...
        if (legResults.length === 0 && !anyLegLost && unsettleableLegCount === 0) {
          console.log(`⏳ Parlay bet ${bet.id.slice(0, 10)}... zero legs resolved — leaving pending (no leg results available yet)`);
          continue;
        }
        ...
        if (anyLegLost) {
          ...
          console.log(`⏳ Parlay bet ${bet.id.slice(0, 10)}... waiting for ${legs.length - legResults.length - unsettleableLegCount} more legs to finish`);
          continue;
        }
        
        // All legs finished (or voided) - settle the parlay
        const parlayWon = !anyLegLost;
        const voidedInfo = unsettleableLegCount > 0 ? ` [${unsettleableLegCount} leg(s) voided — unsettleable market]` : '';
        console.log(`🎰 PARLAY SETTLED: ${bet.id.slice(0, 10)}... ${parlayWon ? 'WON' : 'LOST'} (${legResults.filter(l => l.won).length}/${legs.length} legs won${voidedInfo})`);
        
        const legResultMap = new Map(legResults.map(lr => [String(lr.eventId).trim(), lr]));
        const fullLegResults = legs.map(leg => {
          const eventId = String(leg.eventId).trim();
          const lr = legResultMap.get(eventId);
          if (lr) {
            return {
              eventId: lr.eventId,
              prediction: lr.prediction,
          ...
          return {
            ...
            won: null,
            score: undefined,
            teams: leg.eventName || `${leg.homeTeam || ''} vs ${leg.awayTeam || ''}`.trim() || undefined
          };
        });
        const legResultsJson = JSON.stringify(fullLegResults);
        try {
   ...
   * Falls back to freeSportsResultsCache (batch pull) and disk cache before hitting the API.
   */
  private async fetchFreeSportsLegResult(eventId: string): Promise<FinishedMatch | null> {
    const legacyId = eventId.replace(/_api_/, '_');

    // 1. Check nightly-push in-memory cache (populated by processFreeSportsResults)
    const cached = this.cachedFreeSportsResults.find(r => r.eventId === eventId || r.eventId === legacyId);
    if (cached) {
      console.log(`✅ Found parlay leg ${eventId} in nightly cache (no API call)`);
      return cached;
    }

    // 2. Check batch-pull in-memory cache (populated by fetchFreeSportsResults each settlement cycle)
    if (this.freeSportsResultsCache) {
      const batchCached = this.freeSportsResultsCache.data.find(r => r.eventId === eventId || r.eventId === legacyId);
      if (batchCached) {
        console.log(`✅ Found parlay leg ${eventId} in batch results cache (no API call)`);
        return batchCached;
      }
    }

    // 3. Check disk cache written by previous settlement cycles
    try {
      const fs = require('fs') as typeof import('fs');
      const cacheFile = require('path').join('/tmp', 'free_sports_results_cache.json');
      if (fs.existsSync(cacheFile)) {
        const raw = JSON.parse(fs.readFileSync(cacheFile, 'utf8'));
        const data: any[] = raw.data || raw;
        const diskMatch = data.find((r: any) => r.eventId === eventId || r.eventId === legacyId);
        if (diskMatch) {
          console.log(`✅ Found parlay leg ${eventId} in disk cache (no API call)`);
          return diskMatch as FinishedMatch;
        }
      }
    } catch {}
  ...
  private async fetchFreeSportsGameById(extId: string): Promise<FinishedMatch | null> {
    ...
    const apiKey = ''; // API_SPORTS_KEY disabled — use ESPN for football, ESPN handles all sports
  ...
  private async fetchFreeSportsGameByTeams(extId: string, homeTeam: string, awayTeam: string): Promise<FinishedMatch | null> {
    ...
    const apiKey = ''; // API_SPORTS_KEY disabled — ESPN is the data source
    if (!apiKey) return null;
  ...
  private async settleParlaySingleBet(bet: UnsettledBet, match: FinishedMatch, isWinner: boolean) {
    ...
    const PARLAY_SETTLE_MAX_SBETS = 2_000_000;
    const PARLAY_SETTLE_MAX_SUI = 150;
    const PARLAY_SETTLE_MAX_LBTC = 0.5;   // 0.5 LBTC max parlay payout
    const parlaySettleMax = bet.currency === 'SBETS' ? PARLAY_SETTLE_MAX_SBETS
      : bet.currency === 'LBTC' ? PARLAY_SETTLE_MAX_LBTC
      : PARLAY_SETTLE_MAX_SUI;
  ...
  private evaluateLegPrediction(prediction: string, match: FinishedMatch, marketId?: string, outcomeId?: string): boolean | null {
    ...
    if (earlySpreadPat.test(pred)) {
      console.warn(`⚠️ UNSETTLEABLE POINT SPREAD (parlay leg): prediction="${pred}" — SKIPPING`);
      return null;
    }
    ...
    if (matchesHomeLeg && matchesAwayLeg) {
      ...
      console.warn(`⚠️ AMBIGUOUS PARLAY LEG: prediction="${pred}" matches both "${homeTeam}" and "${awayTeam}" equally — SKIPPING`);
      return null;
    }
    ...
    if (unsettleableKw.some(kw => pred.includes(kw))) {
      console.warn(`⚠️ UNSETTLEABLE PARLAY LEG: prediction="${pred}" — SKIPPING`);
      return null;
    }

    const handicapPattern = /^(home|away|draw|1|2|x)\s*[+-]\d+(\.\d+)?$/i;
    if (handicapPattern.test(pred)) {
      console.warn(`⚠️ UNSETTLEABLE PARLAY HANDICAP: prediction="${pred}" — SKIPPING`);
      return null;
    }
    ...
    if (pred.includes('over')) {
      // Use regex to extract first number after "over" — avoids "1.51.45" parse bug with "Over 1.5 @ 1.45"
      const ouMatch = pred.match(/over\s*([\d.]+)/i);
      const threshold = ouMatch ? parseFloat(ouMatch[1]) : 2.5;
      console.log(`📊 Over/Under eval: pred="${pred}" threshold=${threshold} totalGoals=${totalGoals} → ${totalGoals > threshold ? 'WON' : 'LOST'}`);
      return totalGoals > threshold;
    }
    if (pred.includes('under')) {
      const ouMatch = pred.match(/under\s*([\d.]+)/i);
      const threshold = ouMatch ? parseFloat(ouMatch[1]) : 2.5;
      console.log(`📊 Over/Under eval: pred="${pred}" threshold=${threshold} totalGoals=${totalGoals} → ${totalGoals < threshold ? 'WON' : 'LOST'}`);
      return totalGoals < threshold;
    }
    ...
    console.warn(`⚠️ UNMATCHED PARLAY LEG: prediction="${pred}" did not match any team or market for ${match.homeTeam} vs ${match.awayTeam} — SKIPPING`);
    return null;
  }
    ...
    try {
      ...
      if (neededSports.length > 0) {
        try {
          ...
          // Sync batch-pull results into cachedFreeSportsResults so per-leg parlay lookup can find them
          if (apiResults.length > 0) {
            for (const r of apiResults) {
              if (!this.cachedFreeSportsResults.find(c => c.eventId === r.eventId)) {
                this.cachedFreeSportsResults.push(r);
              }
            }
            console.log(`🔄 Synced ${apiResults.length} batch results into parlay-leg cache (total: ${this.cachedFreeSportsResults.length})`);
          }
        } catch (error) {
      ...
      // Fetch ESPN/TheSportsDB finished events for all free sports (no API key required).
      // This ensures parlay legs placed via ESPN events (raw numeric IDs) are found in
      // finishedMatchMap and can also be matched by team name.
      // Football/soccer uses 96h lookback (vs 72h for other sports) because:
      // 1. European leagues produce results slower on the ESPN free API
      // 2. Bets with numeric API-Sports IDs now rely entirely on team-name
      //    matching against ESPN results — the extra buffer catches stragglers.
      const espnFreeSports = ['baseball', 'hockey', 'basketball', 'mma', 'american-football', 'afl', 'football'];
      for (const sport of espnFreeSports) {
        const lookbackHours = sport === 'football' ? 96 : 72;
        try {
          const espnFinished = await espnSportsService.getFinishedEvents(sport, lookbackHours);
          const espnMatches: FinishedMatch[] = espnFinished
            .filter(e => e.homeScore !== undefined && e.awayScore !== undefined)
            .map(e => {
              const hs = Number(e.homeScore) || 0;
              const as_ = Number(e.awayScore) || 0;
              return {
                eventId: e.id,
                homeTeam: e.homeTeam,
                awayTeam: e.awayTeam,
                homeScore: hs,
                awayScore: as_,
                winner: (hs > as_ ? 'home' : as_ > hs ? 'away' : 'draw') as 'home' | 'away' | 'draw',
                status: 'finished' as const,
              };
            });
          addUnique(espnMatches);
          if (espnMatches.length > 0) {
            console.log(`[Settlement] ESPN finished ${sport}: ${espnMatches.length} matches added`);
          }
        } catch (err: any) {
          console.warn(`[Settlement] ESPN ${sport} finished fetch failed:`, err?.message || err);
        }
      }
  ...
  private async fetchFreeSportsResults(neededSports: string[]): Promise<FinishedMatch[]> {
    ...
    const apiKey = ''; // API_SPORTS_KEY disabled — ESPN handles all sports results
    if (!apiKey) return [];
    ...
    try {
      ...
      return allBets
        ...
        .map(bet => ({
          ...
          placedAt: (typeof bet.createdAt === 'string' ? bet.createdAt : (bet.createdAt as any)?.toISOString?.()) || bet.placedAt || undefined,
          matchDate: (typeof bet.matchDate === 'string' ? bet.matchDate : (bet.matchDate as any)?.toISOString?.()) || undefined,
        }));
    } catch (error) {
  ...
  private static BLOCKED_WALLETS = new Set<string>([
    // Confirmed exploit wallets — real full addresses (May 2026 treasury drain)
  ...
  private async retryPendingPayouts(wonBets: UnsettledBet[]) {
    ...
    for (const bet of wonBets) {
      ...
      try {
        ...
        if (!currentBet || (currentBet.status !== 'won' && currentBet.status !== 'paying')) {
          console.log(`⚠️ PAYOUT SKIP: Bet ${bet.id} no longer in 'won' state (status=${currentBet?.status}) - skipping`);
          this.settledBetIds.add(bet.id);
          continue;
        }
        if (currentBet.status === 'paying') {
          console.log(`⚠️ PAYOUT SKIP: Bet ${bet.id} currently being paid by admin endpoint — skipping to avoid double-pay`);
          continue;
        }
        ...
        if (txHash) {
          ...
          const isAdminClaim = txHash.startsWith('admin-pay-claim-') || txHash.startsWith('admin-correction-');
          const isRealTxHash = !txHash.startsWith('on-chain-') && !isContractSettled && !isAdminClaim;
          if (isRealTxHash) {
          ...
          if (isAdminClaim) {
            console.log(`⚠️ PAYOUT SKIP: Bet ${bet.id} claimed by admin process (${txHash.slice(0,30)}...) - skipping`);
            this.settledBetIds.add(bet.id);
            continue;
          }
        }
        ...
        const RETRY_MAX_STAKE_SBETS = 100_000;
        const RETRY_MAX_STAKE_SUI = 100;
        const RETRY_MAX_STAKE_USDSUI = 1;
        const RETRY_MAX_STAKE_LBTC = 0.1;    // 0.1 LBTC max stake
        const RETRY_MAX_PAYOUT_SBETS = 1_000_000;
        const RETRY_MAX_PAYOUT_SUI = 150;
        const RETRY_MAX_PAYOUT_USDSUI = 4;
        const RETRY_MAX_PAYOUT_LBTC = 0.5;   // 0.5 LBTC max retry payout
        const retryMaxStake = bet.currency === 'SBETS' ? RETRY_MAX_STAKE_SBETS : bet.currency === 'USDSUI' ? RETRY_MAX_STAKE_USDSUI : bet.currency === 'LBTC' ? RETRY_MAX_STAKE_LBTC : RETRY_MAX_STAKE_SUI;
        const retryMaxPay = bet.currency === 'SBETS' ? RETRY_MAX_PAYOUT_SBETS : bet.currency === 'USDSUI' ? RETRY_MAX_PAYOUT_USDSUI : bet.currency === 'LBTC' ? RETRY_MAX_PAYOUT_LBTC : RETRY_MAX_PAYOUT_SUI;
        ...
        if (bet.betObjectId && blockchainBetService.isAdminKeyConfigured()) {
          ...
          if (onChainInfo && !onChainInfo.settled) {
            ...
            if (isGiftBetRetry) {
              ...
              console.log(`🔗 PAYOUT RETRY ON-CHAIN: Bet ${bet.id} (${retryCurrency}) - attempting smart contract settlement`);
              await new Promise(resolve => setTimeout(resolve, 2000));
        ...
        if (bet.currency === 'SUI') {
          ...
          // Unknown/legacy currency (e.g. TD2) with no direct transfer path and no betObjectId
          // Cannot settle — void the bet to stop the retry loop
          console.warn(`⚠️ PAYOUT RETRY: Bet ${bet.id} currency=${bet.currency} has no payout path (no betObjectId, no direct transfer handler) — voiding to stop retry loop`);
          await storage.updateBetStatus(bet.id, 'void', 0);
          this.settledBetIds.add(bet.id);
          continue;
        }
  ...
  private async settleBetsForMatch(match: FinishedMatch, bets: UnsettledBet[]) {
    for (const bet of bets) {
      ...
      if (!bet.stake || bet.stake <= 0) {
        console.log(`⚠️ SKIPPING: Bet ${bet.id} has 0 stake (phantom auto-recovered) — voiding`);
        try {
          await db.execute(sql`UPDATE bets SET status = 'void', settled_at = NOW() WHERE id = ${bet.id} AND status = 'pending'`);
        } catch {}
        this.settledBetIds.add(bet.id);
        continue;
      }
      ...
      try {
          ...
          if (bet.stake > 0) {
            ...
            try {
              if (bet.currency === 'USDSUI') {
                ...
                console.log(`⚠️ VOID: bet ${bet.id} voided — no off-chain refund, on-chain void only`);
              }
            } catch (refundErr: any) {
        ...
        // ANTI-EXPLOIT: Late bet detection — void bets placed after the match should have ended
        // A match starting at matchDate should end by matchDate + 150 minutes (90 min + generous buffer)
        // If the bet was placed after that window, it was placed post-result and must be voided
        if (bet.placedAt && bet.matchDate && bet.status !== 'won') {
          const matchStartMs = new Date(bet.matchDate).getTime();
          const estimatedMatchEndMs = matchStartMs + (150 * 60 * 1000); // 150 min after kickoff
          const betPlacedMs = new Date(bet.placedAt).getTime();
          if (!isNaN(matchStartMs) && !isNaN(betPlacedMs) && betPlacedMs > estimatedMatchEndMs) {
            console.error(`🚨 LATE BET DETECTED: Bet ${bet.id} (${bet.currency}) placed at ${bet.placedAt} — AFTER estimated match end (matchDate=${bet.matchDate}, window closed at ${new Date(estimatedMatchEndMs).toISOString()}). VOIDING.`);
            try {
              await storage.updateBetStatus(bet.id, 'void', 0);
            } catch (voidErr: any) {
              console.error(`❌ Failed to void late bet ${bet.id}:`, voidErr.message);
            }
            this.settledBetIds.add(bet.id);
            continue;
          }
        ...
        const outcomeResult = isAlreadyWon ? true : this.determineBetOutcome(bet, match);
        
        if (outcomeResult === null) {
          console.log(`⏭️ SKIPPING BET ${bet.id}: could not confidently determine outcome for prediction="${bet.prediction}" | ${match.homeTeam} ${match.homeScore}-${match.awayScore} ${match.awayTeam} — leaving unsettled for manual review`);
          continue;
        }
        
        const isWinner = outcomeResult;
        
        if (isAlreadyWon) {
          console.log(`💰 PAYOUT RETRY: Bet ${bet.id} already marked as 'won' - attempting payout`);
        }
        
        console.log(`🎯 SETTLEMENT DECISION: Bet ${bet.id} prediction="${bet.prediction}" → ${isWinner ? 'WON' : 'LOST'} | ${match.homeTeam} ${match.homeScore}-${match.awayScore} ${match.awayTeam} (winner=${match.winner})`);
        ...
        const SETTLEMENT_MAX_PAYOUT_SBETS = 1_000_000;
        const SETTLEMENT_MAX_PAYOUT_SUI = 150;
        const SETTLEMENT_MAX_PAYOUT_USDSUI = 4;
        const SETTLEMENT_MAX_PAYOUT_LBTC = 0.5;           // 0.5 LBTC max settlement
        const settlementMaxPayout = bet.currency === 'SBETS' ? SETTLEMENT_MAX_PAYOUT_SBETS
          : bet.currency === 'USDSUI' ? SETTLEMENT_MAX_PAYOUT_USDSUI
          : bet.currency === 'LBTC'   ? SETTLEMENT_MAX_PAYOUT_LBTC
          : SETTLEMENT_MAX_PAYOUT_SUI;
          ...
          if (statusUpdated) {
            if (isWinner && netPayout > 0) {
              ...
              if (payoutSuccess && payoutTxHash) {
                ...
                console.warn(`⚠️ ON-CHAIN PAYOUT FAILED: Bet ${bet.id} — keeping as 'won' for retry (no off-chain balance)`);
                await storage.updateBetStatus(bet.id, 'won', grossPayout);
              }
            } else {
  ...
  private determineBetOutcome(bet: UnsettledBet, match: FinishedMatch): boolean | null {
    ...
    const fuzzyTeamMatch = (predText: string, teamNorm: string): boolean => {
      ...
      const teamWords = teamNorm.split(/\s+/).filter(w => w.length > 2);
      if (predWords.length === 0 || teamWords.length === 0) return false;
      // Require 65% word overlap (stricter than old 50%) to reduce false positive wins
      const matchCount = teamWords.filter(tw => predWords.some(pw => pw === tw || pw.includes(tw) || tw.includes(pw))).length;
      return matchCount >= Math.max(1, Math.ceil(teamWords.length * 0.65));
    };
    ...
    if (earlySpreadPattern.test(prediction)) {
      console.warn(`⚠️ UNSETTLEABLE POINT SPREAD: prediction="${prediction}" for ${match.homeTeam} vs ${match.awayTeam} — SKIPPING (cannot settle spread as match winner)`);
      return null;
    }
    ...
    if (matchesHome && matchesAway) {
      ...
      if (homeOverlap > awayOverlap) return match.winner === 'home';
      if (awayOverlap > homeOverlap) return match.winner === 'away';
      console.warn(`⚠️ AMBIGUOUS MATCH: prediction="${prediction}" matches both "${homeTeam}" and "${awayTeam}" equally — SKIPPING (leaving unsettled)`);
      return null;
    }
    ...
    if (unsettleableKeywords.some(kw => prediction.includes(kw))) {
      console.warn(`⚠️ UNSETTLEABLE MARKET: prediction="${prediction}" for ${match.homeTeam} vs ${match.awayTeam} — SKIPPING (no period data available)`);
      return null;
    }

    const handicapPattern = /^(home|away|draw|1|2|x)\s*[+-]\d+(\.\d+)?$/i;
    if (handicapPattern.test(prediction)) {
      console.warn(`⚠️ UNSETTLEABLE HANDICAP: prediction="${prediction}" for ${match.homeTeam} vs ${match.awayTeam} — SKIPPING (no handicap handler)`);
      return null;
    }
    ...
    // Use regex to extract the first number after "over"/"under" — avoids
    // "Over 2.5 @ 1.45" being parsed as 2.51.45 → 2.51 by replace(/[^0-9.]/g,'')
    const totalGoals = match.homeScore + match.awayScore;
    if (prediction.includes('over')) {
      const ouMatch = prediction.match(/over\s*([\d.]+)/i);
      const threshold = ouMatch ? parseFloat(ouMatch[1]) : 2.5;
      return totalGoals > threshold;
    }
    if (prediction.includes('under')) {
      const ouMatch = prediction.match(/under\s*([\d.]+)/i);
      const threshold = ouMatch ? parseFloat(ouMatch[1]) : 2.5;
      return totalGoals < threshold;
    }
    ...
    console.warn(`⚠️ UNMATCHED PREDICTION: prediction="${prediction}" did not match any team or market for ${match.homeTeam} (${match.homeScore}) vs ${match.awayTeam} (${match.awayScore}) — SKIPPING (leaving unsettled)`);
    return null;
  }
  ...
  async manualSettle(betId: string, outcome: 'won' | 'lost' | 'void') {
    try {
      ...
      if (outcome === 'won' && payout > 0) {
        if (!bet.userId || !/^0x[0-9a-fA-F]{64}$/.test(bet.userId)) {
          console.error(`❌ SETTLEMENT FAILED: Bet ${betId} has invalid userId '${bet.userId?.slice(0,20)}' - cannot process payout`);
          return { success: false, betId, outcome, payout, message: 'Bet has no valid wallet userId' };
        }
        console.log(`💰 MANUAL SETTLE: ${bet.userId.slice(0,12)}... won ${payout} ${bet.feeCurrency} — on-chain payout only, no off-chain balance`);
  ...
  async forceOnChainSettlement(betId: string, outcome: 'won' | 'lost' | 'void'): Promise<{ success: boolean; txHash?: string; error?: string }> {
    try {
      ...
      // PRE-CHECK: Verify bet isn't already settled on-chain before submitting a tx
      const existingInfo = await blockchainBetService.getOnChainBetInfo(betObjectId);
      if (existingInfo?.settled) {
        console.log(`⚠️ FORCE ON-CHAIN SETTLEMENT: Bet ${betId} already settled on-chain (status=${existingInfo.status}) — no duplicate tx sent`);
        return { success: true, txHash: `already-settled-on-chain-${betObjectId.slice(0, 16)}` };
      }
      if (!existingInfo) {
        console.warn(`⚠️ FORCE ON-CHAIN SETTLEMENT: Bet object ${betObjectId} not found on-chain`);
        return { success: false, error: `Bet object ${betObjectId} not found on-chain` };
      }
