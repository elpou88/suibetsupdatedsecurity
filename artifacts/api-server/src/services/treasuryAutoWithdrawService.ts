...
async function attemptAutoWithdraw(): Promise<{
  ...
  // SECURITY: single-key withdraw_fees is blocked by default to prevent an
  // admin-key compromise from draining the fee vault directly.
  // It must be explicitly re-enabled by setting ALLOW_SINGLE_KEY_WITHDRAW=true.
  if (process.env.ALLOW_SINGLE_KEY_WITHDRAW !== 'true') {
    console.log('[TreasuryAutoWithdraw] ⛔ Single-key withdrawals are disabled by default. Use the multi-sig propose → approve → execute flow. To override in emergencies only, set ALLOW_SINGLE_KEY_WITHDRAW=true.');
    return result;
  }
