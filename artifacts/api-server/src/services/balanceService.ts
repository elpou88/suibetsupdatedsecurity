...
export class BalanceService {
   ...
   * PERSISTS TO DATABASE
   * 3% of ALL revenue is automatically routed to SBETS buyback & burn pool
   */
  async addRevenue(amount: number, currency: 'SUI' | 'SBETS' = 'SUI', source: 'lost_bet' | 'won_bet_fee' | 'voided_bet' | 'manual' = 'lost_bet', betId?: string): Promise<void> {
    let buybackAmount = 0;
    try {
      const { buybackService } = await import('./buybackService');
      buybackAmount = buybackService.addRevenueToBuyback(amount, currency);
    } catch {}

    const netRevenue = buybackAmount > 0 ? amount - buybackAmount : amount;
    await storage.addPlatformRevenue(netRevenue, currency);
    
    try {
      const { revenueTrackerService } = await import('./revenueTrackerService');
      await revenueTrackerService.recordRevenue(source, amount, buybackAmount, currency, betId);
    } catch {}

    if (buybackAmount > 0) {
      console.log(`📊 REVENUE ADDED (DB): ${netRevenue.toFixed(4)} ${currency} to platform (${buybackAmount.toFixed(4)} ${currency} → buyback & burn pool)`);
    } else {
      console.log(`📊 REVENUE ADDED (DB): ${amount} ${currency} to platform`);
    }
  }
