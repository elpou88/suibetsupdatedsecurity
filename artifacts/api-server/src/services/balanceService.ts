...
export class BalanceService {
   ...
   * PERSISTS TO DATABASE
   */
  async addRevenue(amount: number, currency: 'SUI' | 'SBETS' | 'USDC' | 'USDSUI' = 'SUI', source: 'lost_bet' | 'won_bet_fee' | 'voided_bet' | 'manual' = 'lost_bet', betId?: string): Promise<void> {
    await storage.addPlatformRevenue(amount, currency);

    try {
      const { revenueTrackerService } = await import('./revenueTrackerService');
      await revenueTrackerService.recordRevenue(source, amount, currency, betId);
    } catch {}
