...
// RAILWAY_DATABASE_URL takes priority in production (Railway) over the local DATABASE_URL
const pool = new pg.Pool({ connectionString: process.env.RAILWAY_DATABASE_URL || process.env.DATABASE_URL });
...
export class DatabaseStorage implements IStorage {
  ...
  async createBet(bet: any): Promise<any> {
    try {
      ...
      // Use the real blockchain tx hash from the bet object; null if not yet confirmed
      const txHash = bet.txHash || null;
      ...
      const [inserted] = await db.insert(bets).values({
        ...
        leagueName: bet.leagueName || null,
        sportName: bet.sportName || null,
  ...
  async updateBetStatus(betId: string, status: string, payout?: number, settlementTxHash?: string): Promise<boolean> {
    try {
      ...
      const settledAt = (status === 'won' || status === 'lost' || status === 'void') 
