...
export const socialPredictions = pgTable("social_predictions", {
  ...
  yesReserve: real("yes_reserve").default(10000),
  noReserve: real("no_reserve").default(10000),
  initialLiquidity: real("initial_liquidity").default(10000),
  resolutionSource: text("resolution_source").default("creator"),
  totalVolume: real("total_volume").default(0),
  creatorResolution: text("creator_resolution"),
...
export const socialPredictionBets = pgTable("social_prediction_bets", {
  ...
  shares: real("shares"),
  betType: text("bet_type").default("buy"),
  status: text("status").default("active"),
});
...
export const socialPredictionComments = pgTable("social_prediction_comments", {
  id: serial("id").primaryKey(),
  predictionId: integer("prediction_id").notNull(),
  wallet: text("wallet").notNull(),
  message: text("message").notNull(),
...
export const p2pBetOffers = pgTable("p2p_bet_offers", {
  ...
  takerStake: real("taker_stake").notNull(),
  currency: text("currency").default("SUI"),
  filledStake: real("filled_stake").default(0),
  status: text("status").default("open"),
  ...
  // On-chain contract fields — populated when the offer is posted on-chain
  ...
  deepbookClientOrderId: text("deepbook_client_order_id"),  // numeric clientOrderId sent to SDK
  deepbookOrderId: text("deepbook_order_id"),               // on-chain order ID (u64) for cancellation
  deepbookOrderDigest: text("deepbook_order_digest"),        // tx digest of the DeepBook post
  // ── Sui-native feature extensions (2025-2026) ─────────────────────────────
  shareToken: text("share_token").unique(),                  // zkSend challenge link token (UUID)
  suinsGated: boolean("suins_gated").default(false),         // VIP pool: taker must own a .sui name
  liveOdds: boolean("live_odds").default(false),             // In-play: odds shift with live score
  scoreSnapshot: text("score_snapshot"),                     // "homeScore-awayScore" at creation time (live offers)
  matchMinute: integer("match_minute"),                      // Match minute when offer was posted (live offers)
});
...
export const p2pParlayOffers = pgTable("p2p_parlay_offers", {
  ...
  makerRebateRate: real("maker_rebate_rate").default(0),
  actualPayout: real("actual_payout"),
  // On-chain contract fields — populated when the parlay is posted on-chain
